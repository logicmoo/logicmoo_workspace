...
% fact(gate(x1,xor)).
prove_gate(x1, xor, _, _).
ex_gate(x1, xor, ths(A, A, B, B), _, ans(C, C)).

% fact(gate(x2,xor)).
prove_gate(x2, xor, _, _).
ex_gate(x2, xor, ths(A, A, B, B), _, ans(C, C)).

% fact(gate(a1,and)).
prove_gate(a1, and, _, _).
ex_gate(a1, and, ths(A, A, B, B), _, ans(C, C)).

% fact(gate(a2,and)).
prove_gate(a2, and, _, _).
ex_gate(a2, and, ths(A, A, B, B), _, ans(C, C)).

% fact(gate(o1,or)).
prove_gate(o1, or, _, _).
ex_gate(o1, or, ths(A, A, B, B), _, ans(C, C)).

% fact(conn(in(1,f1),in(1,x1))).
prove_conn(in(1, f1), in(1, x1), _, _).
ex_conn(in(1, f1), in(1, x1), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(in(1,f1),in(1,a1))).
prove_conn(in(1, f1), in(1, a1), _, _).
ex_conn(in(1, f1), in(1, a1), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(in(2,f1),in(2,x1))).
prove_conn(in(2, f1), in(2, x1), _, _).
ex_conn(in(2, f1), in(2, x1), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(in(2,f1),in(2,a1))).
prove_conn(in(2, f1), in(2, a1), _, _).
ex_conn(in(2, f1), in(2, a1), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(in(3,f1),in(2,x2))).
prove_conn(in(3, f1), in(2, x2), _, _).
ex_conn(in(3, f1), in(2, x2), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(in(3,f1),in(1,a2))).
prove_conn(in(3, f1), in(1, a2), _, _).
ex_conn(in(3, f1), in(1, a2), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(out(1,x1),in(1,x2))).
prove_conn(out(1, x1), in(1, x2), _, _).
ex_conn(out(1, x1), in(1, x2), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(out(1,x1),in(2,a2))).
prove_conn(out(1, x1), in(2, a2), _, _).
ex_conn(out(1, x1), in(2, a2), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(out(1,a1),in(2,o1))).
prove_conn(out(1, a1), in(2, o1), _, _).
ex_conn(out(1, a1), in(2, o1), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(out(1,a2),in(1,o1))).
prove_conn(out(1, a2), in(1, o1), _, _).
ex_conn(out(1, a2), in(1, o1), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(out(1,x2),out(1,f1))).
prove_conn(out(1, x2), out(1, f1), _, _).
ex_conn(out(1, x2), out(1, f1), ths(A, A, B, B), _, ans(C, C)).

% fact(conn(out(1,o1),out(2,f1))).
prove_conn(out(1, o1), out(2, f1), _, _).
ex_conn(out(1, o1), out(2, f1), ths(A, A, B, B), _, ans(C, C)).

% fact(val(in(_17978,_17980),anything)).
prove_val(in(_, _), anything, _, _).
ex_val(in(_, _), anything, ths(A, A, B, B), _, ans(C, C)).

% default(ok(_18514)).
prove_ok(A, B, _) :-
	member(ok(A), B).
ex_ok(D, ths(A, A, B, B), _, ans(C, C)) :-
	member(ok(D), A).
ex_ok(A, ths(B, [ok(A)|B], C, C), _, ans(D, D)) :-
	variable_free(ok(A)),
	\+ member(ok(A), B),
	\+ prove_not_ok(A, [ok(A)|B], anc([], [])).
ex_ok(B, ths(A, A, C, [ok(B)|C]), _, ans(D, D)) :-
	\+ variable_free(ok(B)).
...
prove_val(out(1, A), C, B, anc(D, E)) :-
	...
	prove_val(in(2, A),
		  ...
		  B,
		  anc([val(out(1, A), C)|D], E)).
ex_val(out(1, A), D, ths(B, U, C, W), anc(E, F), ans(G, Y)) :-
	ex_ok(A,
	      ths(B, H, C, I),
...
prove_faulty(A, B, _) :-
	member(faulty(A), B).
ex_faulty(D, ths(A, A, B, B), _, ans(C, C)) :-
	member(faulty(D), A).
ex_faulty(A, ths(B, [faulty(A)|B], C, C), _, ans(D, D)) :-
	variable_free(faulty(A)),
	\+ member(faulty(A), B),
	\+ prove_not_faulty(A, [faulty(A)|B], anc([], [])).
ex_faulty(B, ths(A, A, C, [faulty(B)|C]), _, ans(D, D)) :-
...
prove_val(out(1, A), C, B, anc(D, E)) :-
	...
	prove_val(in(2, A),
		  ...
		  B,
		  anc([val(out(1, A), C)|D], E)).
ex_val(out(1, A), D, ths(B, Y, C, A1), anc(E, F), ans(G, C1)) :-
	ex_faulty(A,
		  ths(B, H, C, I),
		  anc([val(out(1, A), D)|E], F),
		  ans(G, J)),
...
prove_not_ok(A, B, anc(C, D)) :-
	prove_faulty(A, B, anc(C, [ok(A)|D])).
ex_not_ok(A, B, anc(C, D), E) :-
	ex_faulty(A, B, anc(C, [ok(A)|D]), E).
prove_not_faulty(A, B, anc(C, D)) :-
	prove_ok(A, B, anc(C, [faulty(A)|D])).
ex_not_faulty(A, B, anc(C, D), E) :-
	ex_ok(A, B, anc(C, [faulty(A)|D]), E).

% fact(opp(on,off)).
prove_opp(on, off, _, _).
ex_opp(on, off, ths(A, A, B, B), _, ans(C, C)).

% fact(opp(off,on)).
prove_opp(off, on, _, _).
ex_opp(off, on, ths(A, A, B, B), _, ans(C, C)).

% fact(ttable(and,on,on,on)).
prove_ttable(and, on, on, on, _, _).
ex_ttable(and, on, on, on, ths(A, A, B, B), _, ans(C, C)).

% fact(ttable(and,off,anything,off)).
prove_ttable(and, off, anything, off, _, _).
ex_ttable(and, off, anything, off, ths(A, A, B, B), _, ans(C, C)).

% fact(ttable(and,anything,off,off)).
prove_ttable(and, anything, off, off, _, _).
ex_ttable(and, anything, off, off, ths(A, A, B, B), _, ans(C, C)).

% fact(ttable(or,off,off,off)).
prove_ttable(or, off, off, off, _, _).
ex_ttable(or, off, off, off, ths(A, A, B, B), _, ans(C, C)).

% fact(ttable(or,on,anything,on)).
prove_ttable(or, on, anything, on, _, _).
ex_ttable(or, on, anything, on, ths(A, A, B, B), _, ans(C, C)).

% fact(ttable(or,anything,on,on)).
prove_ttable(or, anything, on, on, _, _).
ex_ttable(or, anything, on, on, ths(A, A, B, B), _, ans(C, C)).

% fact(ttable(xor,off,on,on)).
prove_ttable(xor, off, on, on, _, _).
ex_ttable(xor, off, on, on, ths(A, A, B, B), _, ans(C, C)).

% fact(ttable(xor,off,off,off)).
prove_ttable(xor, off, off, off, _, _).
ex_ttable(xor, off, off, off, ths(A, A, B, B), _, ans(C, C)).

% fact(<-(ttable(xor,on,_3386,_3388),opp(_3386,_3388))).
prove_ttable(xor, on, A, B, C, anc(D, E)) :-
	prove_opp(A,
		  B,
		  C,
		  anc([ttable(xor, on, A, B)|D], E)).
ex_ttable(xor, on, A, B, C, anc(D, E), F) :-
	ex_opp(A,
	       B,
	       C,
	       anc([ttable(xor, on, A, B)|D], E),
...
prove_val(C, A, B, anc(D, E)) :-
	...
	prove_val(F,
		  ...
		  B,
		  anc([val(C, A)|D], E)).
ex_val(D, A, ths(B, M, C, O), anc(E, F), ans(G, Q)) :-
	ex_ne(A,
	      anything,
	      ths(B, H, C, I),
	      anc([val(D, A)|E], F),
...
ex_ne(D, E, ths(A, A, B, B), _, ans(C, C)) :-
	ne(D, E).
prove_ne(A, B, _, _) :-
	ne(A, B).
...
prove_not_val(A, off, B, anc(C, D)) :-
	prove_val(A, on, B, anc(C, [val(A, off)|D])).
...
prove_not_val(A, on, B, anc(C, D)) :-
	prove_val(A, off, B, anc(C, [val(A, on)|D])).
...
prove_diag(A, C, B, anc(D, E)) :-
	...
	prove_val(out(2, f1),
		  ...
		  B,
		  anc([diag(A, C)|D], E)).
ex_diag(A, D, ths(B, I, C, K), anc(E, F), ans(G, M)) :-
	ex_val(out(1, f1),
	       A,
	       ths(B, H, C, J),
	       anc([diag(A, D)|E], F),
