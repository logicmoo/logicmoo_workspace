%%% Interpreter for the B language in B Prolog
...
%'15puzzle_rev.txt'   %%% ASP competition 2009
'C:/AGOSTINO/CODICI_VARI/BLANGUAGE/tangram_jul10.pl'
%'barrels.txt'
%'puzzle.txt'
%'orderedBlocks.txt'
%'capracavolo.txt'
...
my_labeling(Varsocc,States) :-
    ...
    %%%neg_vars(Varsocc,NegVarsocc), %%% To simulate "down"
...
%%% C is the "or" of all elements of A and of B
bool_or(A,B,C) :- 
    append(A,B,D),
    bool_or(D,C).

%%% COMMENTED for BPRolog 
...
table(VarLists, Tuples) :-
        listsTotuples(VarLists,VarTuples),
        VarTuples in Tuples.  

listsTotuples([], []).
listsTotuples([L|Ls], [T|Ts]) :-
        list_to_and(L, T),
        listsTotuples(Ls,Ts).
