...
convert_clauses_to_file(F) :-
	...
	((Type == comment ; Type == skip) ->
	  ...
	  printf(F,"%%%% %w %Dw\n",[Typestring,FinalFormula]),
	   ...
	   ; true),
%	  strip_sort(Clauses, SClauses),
	  SClauses = Clauses,
	  write_clauses(F,SClauses,Type),
	  printf(F,"\n",[])),
	convert_clauses_to_file(F).
...
%clausify(F,Res,SkolemFunctions) :-
%	nnf(F,Fnnf),
%	skolemize(Fnnf,[],Matrix,SkolemFunctions),
%	cnf(Matrix,Mcnf),
%	norm_con(Mcnf,Res), !.
%
...
subst(X/T,(A,B),(SA,SB)) :- subst(X/T,A,SA), subst(X/T,B,SB).
subst(X/T,(A;B),(SA;SB)) :- subst(X/T,A,SA), subst(X/T,B,SB).
subst(X/T,(A v B),(SA;SB)) :- subst(X/T,A,SA), subst(X/T,B,SB).
