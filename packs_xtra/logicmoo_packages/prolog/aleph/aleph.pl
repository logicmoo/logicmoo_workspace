...
init(yap):-
	source,
	system_predicate(nil,nil), hide(nil),
...
% :- redefine_system_predicate(nil/0).
...
:- 	dynamic(nil/0),
	...
	use_module(library(broadcast)),
        ...
        (predicate_property(time:alarm(_,_,_),_) ->
		use_module(library(time));
                (redefine_system_predicate(time:alarm/3),
                   abolish(time:alarm/3),
                   assert(alarm(_,_,_)),
                   redefine_system_predicate(time:remove_alarm/1),
                   abolish(time:remove_alarm/1),
...
flatten(Depth,MaxDepth,Last,Last1):-
	...
	'$aleph_sat_atom'(_,_), !,
	(setting(permute_bottom,Permute) -> true; Permute = nil),
...
flatten_atom(Depth,Depth1,Lit,Negated,Mode,Last,Last1):-
	...
	(Depth = Depth1 -> CheckOArgs = true; CheckOArgs = nil),
...
update_lit(LitNum,nil,FAtom,I,O,D):-
...
mark_lit(Lit,Depth,GetPreds,VSoFar,P1,V1):-
	...
	(GetPreds = nil ->
		...
		mark_lits(D1,Depth,nil,[],_,VSoFar,_),
...
add_eqs([Var2|Rest],Depth,Var1,Type,Last,NewLast):-
	(Depth = 0 -> 
		add_lit(Last,nil,(Var1=Var2),[1/Type],[2/Type],[Var1],[Var2],Last1);
		add_lit(Last,nil,(Var1=Var2),[1/Type,2/Type],[],[Var1,Var2],[],Last1)),
...
graphsearch(S,Nodes):-
	...
	next_node(NodeRef),
        ...
        ((LazyPreds = []; RefineOp \= nil)  -> Succ1 = Succ;
...
get_search_settings(S):-
	...
	setting(openlist,OSize), arg(25,S,OSize),
        ...
        ('$aleph_sat'(eq,Eq) -> arg(27,S,Eq); arg(27,S,nil)),
	...
	(get_ovars1(nil,1,HIVars) ->  arg(31,S,HIVars); arg(31,S,[])),
	...
	(setting(rls_type,RlsType) -> arg(39,S,RlsType);arg(39,S,nil)),
	setting(minposfrac,MinPosFrac), arg(40,S,MinPosFrac),
	(setting(recursion,Recursion) -> true; Recursion = nil),
	...
	(setting(construct_features,Features)-> arg(44,S,Features); arg(44,S,nil)),
...
expand(nil,S,NodeRef,NodeRef,Path1,Length,Descendents,PosCover,NegCover,OVars,C,TV,CL):-
	!,
        '$aleph_search_node'(NodeRef,LitNum,Path,Length/_,PCover,NCover,OVars,_),
	arg(46,S,SSample),
	(SSample = nil -> PosCover = PCover, NegCover = NCover;
...
get_sibgain(S,LVars,LitNum,Desc,Node,LastSib,Last,Best,Path,C,TV,L,Min,Pos,Neg,OVars,E,LBest,LNode):-
        ...
        get_ivars1(nil,Sib,SibIVars),
        (intersects(SibIVars,LVars) -> Flag = upper;
                get_ovars1(nil,Sib,SibOVars),
...
get_theory_gain1(S,Theory,Last,Best,Pos,Neg,P,N,Best1):-
        (nil -> p_message('constraint violated'),
                Contradiction = true;
                Contradiction = nil),
	Contradiction = nil,
        Node1 is Last + 1,
...
get_gain1(S,Flag,C,CL,EMin/EL,Last,Best/Node,Path,L1,Pos,Neg,OVars,E,Best1):-
	...
	arg(22,S,Search),
        ((Search \== ic, nil) -> p_message('constraint violated'),
                Contradiction = true;
                Contradiction = nil),
        Node1 is Last + 1,
        arg(8,S,Caching),
	...
	arg(10,S,LCost),
	arg(11,S,LContra),
        ((Verbosity >= 1, LContra = nil, LCost = nil) ->
		Label = [A,B|_],
		p_message(A/B);
	true),
        arg(7,S,ClauseLength),
	(RefineOp = nil ->
		get_ovars1(nil,L1,OVars1),
		aleph_append(OVars1,OVars,OVars2);
		true),
        ((ClauseLength=CL, RefineOp = nil) -> true;
		(RefineOp = nil ->
	...
	(VSearch = true ->
		...
		true),
        (((RefineOp \= nil,Contradiction=nil);
...
% also, only checks for constraints of the form nil:- ...
% this simplifies the check of Body,not(Head) to just Body
ccheck(S,(nil:-Body),[],[0,N|_]):-
	(Body = true ->
		N is inf;
		arg(11,S,LContra),
		(LContra = nil -> 
...
prove_cache(exact,S,Type,Entry,Clause,Intervals,Max,IList,Count):-
	...
	(Intervals = Exact/Left ->
		...
		Max1 is Max - Count0,
        	...
        	arg(29,S,Time),
        	arg(34,S,Proof),
        	prove(LNegs/nil,Depth/Time/Proof,Type,Clause,Left,Max1,IList1,Count1),
...
index_prove1(nil/_,_,_,_,_,_,Proved,Allowed,_):-
        Proved > Allowed, !, fail.
...
find_tree(Type):-
	...
	p1_message('construction timeit'), p_message(Time).
...
add_model(Evalfn,Clause,PredictArg,Examples,_,_,_):-
	...
	split_clause(Clause,Head,_),
	tparg(PredictArg,Head,Var),
	asserta('$aleph_local'(tree_model,nil,0,Best)),
...
get_best_subtree(nil,_,Gain,Left,Right):-
...
sat(Type,Num):-
	...
	p1_message('saturation timeit'), p_message(Time),
...
reduce(ibs):-
	...
	('$aleph_sat'(example,example(Num,Type)) ->
		...
		asserta('$aleph_search'(ibs_selected,selected(Label,(Example:-true),
				[Num-Num],[])));
		asserta('$aleph_search'(ibs_selected,selected(Label,(nil:-true),
				[],[])))),
	stopwatch(Start),
	repeat,
	...
	p1_message('ibs search timeit'), p_message(Time),
...
reduce(id):-
	...
	('$aleph_sat'(example,example(Num,Type)) ->
		...
		asserta('$aleph_search'(id_selected,selected(Label,(Example:-true),
				[Num-Num],[])));
		asserta('$aleph_search'(id_selected,selected(Label,(nil:-true),
				[],[])))),
	stopwatch(Start),
	repeat,
	...
	p1_message('id search timeit'), p_message(Time),
...
reduce(ils):-
	...
	('$aleph_sat'(example,example(Num,Type)) ->
		...
		asserta('$aleph_search'(ils_selected,selected(Label,(Example:-true),
				[Num-Num],[])));
		asserta('$aleph_search'(ils_selected,selected(Label,(nil:-true),
				[],[])))),
	stopwatch(Start),
	repeat,
	...
	p1_message('ils search timeit'), p_message(Time),
...
reduce(rls):-
	...
	(Refine \= nil  -> true; set(refineop,rls)),
	...
	p1_message('rls search timeit'), p_message(Time),
...
reduce(scs):-
	...
	p1_message('scs search timeit'), p_message(Time),
...
reduce(ic):-
	...
	(RefineOp = nil -> set(refineop,auto); true),
	set(minpos,0),
	set(searchstrat,bf),
	set(evalfn,coverage),
	set(explore,true),
...
find_clause(Search):-
	...
	('$aleph_sat'(example,example(Num,Type)) ->
		...
		asserta('$aleph_search'(selected,selected(Label,(Example:-true),
							[Num-Num],[])));
		asserta('$aleph_search'(selected,selected(Label,(nil:-true),[],[])))),
	arg(13,S,MinPos),
	...
	p1_message('search timeit'), p_message(Time),
	p_message('best clause'),
	pp_dclause(RClause),
	show_stats(Evalfn,BestLabel),
...
find_theory(rls):-
	...
	setting(evalfn,Evalfn),
        complete_label(Evalfn,[0-[0,0,[],nil]],[P0,N0,1],Label),
	asserta('$aleph_search'(rls_selected,selected(Label,[0-[0,0,[],nil]],
						PosSet,NegSet))),
...
find_theory1(_):-
	...
	arg(4,S,_/Evalfn),
        interval_count(Pos,P),
        interval_count(Neg,N),
        complete_label(Evalfn,[0-[0,0,[],nil]],[P,N,1],Label),
	asserta('$aleph_search'(selected,selected(Label,[0-[0,0,[],nil]],Pos,Neg))),
	get_theory_gain(S,0,Label/0,[0-[0,0,[],nil]],Pos,Neg,P,N,NextBest,Last),
...
% constructs theories 1 clause at a timeit
induce_clauses:-
...
induce:-
	...
	retractall('$aleph_global'(besthyp,besthyp(_,_,_,_,_))),
        asserta('$aleph_global'(besthyp,besthyp([-inf,0,1,-inf],0,(nil),[],[]))),
        get_besthyp(Abduce),
	...
	reinstate(portray_search),
        p1_message('timeit taken'), p_message(Time), 
	show_total_stats,
	record_total_stats, !.
induce.

% construct theories 1 clause at a timeit
...
% A constraint is a clause of the form nil:-... 
...
soldnf_solve(not(Goal),true,SkippedSoFar,Skipped):-
	soldnf_solve(Goal,nil,SkippedSoFar,Skipped).
soldnf_solve(not(Goal),nil,SkippedSoFar,Skipped):-
	!,
...
% all #'d arguments of these literals are evaluated at reduction-timeit
...
evaluate(OldLit,_,Lit,I,O,D):-
	...
	update_lit(LitNum,nil,NewLit,I,O,D),
...
show_atoms_left:-
	...
	p1_message('estimated timeit to finish (secs)'), p_message(EstTime), !.
show_atoms_left.
...
sample_clause_using_modes(D,L,Clause):-
	findall(H,auto_refine(nil,H),HL),
	HL \= [],
...
rls_refine(clauses,_-[_,_,_,nil],Clause):-
	!,
...
rls_refine(theories,[_-[_,_,_,nil]],Theory):-
	!,
...
%most of the timeit no intersection, so optimise on that
...
check_auto_refine:-
	(setting(construct_bottom,reduction);setting(construct_bottom,nil)),
...
set_lazy_on_contradiction(_,_):-
	'$aleph_global'(lazy_on_contradiction,set(lazy_on_contradiction,nil)), !.
...
construct_name(_,Name,Name):-
	setting(use_file_extensions,nil), !.
...
%	Flag is one of "true" or "nil" depending on whether bottom
%	requires storing
bottom_key(N,T,Key,Flag):-
	((var(N),var(T)) ->
...
reset:-
	...
	assert_static(example(0,uspec,nil)),
	set_default(_),
	!.
...
set_def(construct_bottom, saturation,
	...
	[saturation, reduction, nil], saturation,
	show).
...
set_def(search, search-search_strategy,
	'Search Strategy',
	[bf, df, heuristic, ibs, ils, rls, scs, id, ic, ar, nil], bf,
	show).
...
special_consideration(record,true):-
	...
	(setting(recordfile,F) -> 
		...
		true), !.
special_consideration(record,nil):-
	noset(recordfile_stream), !.
...
special_consideration(good,true):-
	...
	(setting(goodfile,F) -> 
		...
		true), !.
special_consideration(good,nil):-
	noset(goodfile_stream), !.
