...
sub PrintTestingFromArray
{
	...
	my @edges = ();
	foreach my $m (@syms_nrs)
	{
	    push(@edges, "$m:$m") if($m  >= $gsymoffset);
	}

	my $training_exmpl = join(",", @edges);

	print TEST ($#edges, ';', $training_exmpl, ',', $grefnr{$ref}, ":\n"); 
...
#    LoadSpecs(); # calls LoadTables too
    ...
    my $evalfile = ($greuseeval > 0) ? "$filestem.eval_$iter1" : "$filestem.eval_$iter1"; #"/dev/null";
	...
	my $snow_pid = (($greuseeval > 0) &&
		...
		open(SOUT,"python src/main.py --train=$filestem.alltrain_$previter --test=$filestem.test_$iter1  --all=$filestem.alltrain_$previter -l $wantednr |tee $evalfile|");
...
sub Iterate
{
	...
	print "trained 0\n";
	# die "";
	`cp $filestem.train_0 $filestem.alltrain_0`;
...
sub PrintTrainingFromHash
{
	...
	my @edges = ();
	foreach my $m (@all_nrs)
	{
	    push(@edges, "$m:$m") if($m  >= $gsymoffset);
	}

	my $training_exmpl = join(",", (@edges, @refs_nrs));
	print TRAIN ($#edges, ';', $training_exmpl, ":\n"); 
