...
close(PROT);
print "min: ";
foreach $prot (@prot) { $prot=~s/protokoll_//; $prot=~tr/\-/_/; $prot=~tr/\///; print " +1",$prot; }
print ";\n";
open(PROB,"00SOLVED");
while(<PROB>) 
{
    chop;
    @matched=`grep -l $_ protoko*`;
    foreach $prot (@matched) { chop($prot); $prot=~s/protokoll_//; $prot=~tr/\-/_/; $prot=~tr/\///; print " +1",$prot; }
    print ">= 1;\n";
} 
