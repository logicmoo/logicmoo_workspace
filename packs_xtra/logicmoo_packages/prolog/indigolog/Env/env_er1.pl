...
%  This is done with an alarm/3 from library(alarm)
%
% This environment is self-contained (automatically it loads the required
%  libraries). It should be called as follows:
%
%   pl host=<HOST> port=<PORT> -b env_er1.pl -e start
%		iper1=<ER1HOST> porter1=<ER1PORT>
%
% where HOST/PORT is the address of the environment manager socket and
% ER1HOST/ER1PORT is the address of the ER1 robot server
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%                             November 15, 2002
%
...
% whenever possible.
:- dynamic alreadySeen/2, checkLostEvery/1.
...
%        initializeER1_Events,
        ...
        report_message(system(3),'Tracking of objects initialized successfully'),
...
% Set the alarm process to check for objects that have been seen
initializeObjectTracking :-
        checkLostEvery(CheckEverySeconds),
        CheckEverySeconds\=0,
		alarm(CheckEverySeconds, checkLostObjects, _Id, [remove(true)]).
%		assert(checkLostObjectsProcessId(_Id)).
initializeObjectTracking.
...
% Set the alarm process to check for objects that have been seen
finalizeObjectTracking :-
		retract(checkLostEvery(_)),
		assert(checkLostEvery(0)),
		current_alarm(_At, _Callable, Id, _Status),
		catch_succ(remove_alarm(Id),'').
...
% Handler for tracking lost objects
%	Will re-post after finished
checkLostObjects :- checkLostObjects2, fail.
checkLostObjects :- initializeObjectTracking.	% Repost object tracking
