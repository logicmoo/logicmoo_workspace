...
Examples
   ...
   Success:
   fluent_instance(atom,X).
   fluent_instance(f(a,b),X).
   fluent_instance(f(a,b),f(X,Y)).
   fluent_instance(f(a,X),f(Y,X)).
   fluent_instance(f(a,X),f(X,Y)).
   fluent_instance(f(X,Y),f(Y,X)).
   fluent_instance([a,b,c],[A,B,C]).
   fluent_instance([a,f(1,b,X),Y|Z],T).
   X::1..5, fluent_instance(3,X).
   X::2..4, Y::1..5, fluent_instance(X,Y).

   Fail:
   fluent_instance(f(a,b),f(X,X)).
   fluent_instance(X,a).
   X::2..4, Y::1..5, fluent_instance(Y,X).
*/

% fluent_instance(X,Y):-!,subsumes_term(xyz(X),xyz(Y)).
fluent_instance(X,Y):-not(not((X=@=Y,contains_term(xyz(X),xyz(Y))))).
