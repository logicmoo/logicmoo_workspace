...
load_unity_prolog_file(F):- 
  ...
  load_files(FN,[module(lmchat),must_be_module(false),redefine_module(false),scope_settings(false)]).

unity_prolog_filename(F,FN):- exists_file(F),!,FN=F.
unity_prolog_filename(F,FN):- lmchat_dir(D),atomic_list_concat([D,'/',F],FN),exists_file(FN),!.
