...
test(assrt_lib_1) :-
    ...
    assertion(R=[assertions:assertion_db(p(A, B),
					 m, check, pred,
					 [int(A)],
					 [gnd(A), var(B)],
					 [gnd(A), gnd(B)],
...
test(assrt_lib_simple) :-
    assrt_lib:assrt_lib_tr((:- pred atomic_list_concat(A, B)
			   ...
			   Record, a, Dict),
    assertion(Record=[assertions:assertion_db(atomic_list_concat(A, B),
					      a,
					      check, pred,
					      [list(A, ground), atom(B)],
					      [list(A, atom),   term(B)],
...
test(assrt_lib_comp) :-
    ...
    assertion(R=[assertions:assertion_db(nfi(G, V), m, true, comp, [], [], [],
...
test(assrt_lib_multi) :-
    ...
    assertion(Records=[assertions:assertion_db(a(A),    q, check, pred, [], [],
					       [], [m:kbrule, kbmask([+])], "", []),
		       assertions:assertion_db(b(A, _B), m, check, pred, [], [],
					       [], [kbrule, hidden], "", [])]).

test(assrt_lib_mode1) :-
    assrt_lib:assertion_records(pred dict(+int),Re, u,[]),
    assertion(Re=[assertions:assertion_db(dict(A), u, check, pred, [],
					  [int(A)], [], [], "", [])]).
...
test(assrt_lib_oddity_1) :-
    ...
    assertion(R=[assertions:assertion_db(b(A, B), a, check, pred,
					 [], [e(A), l(B)], [], [n], "", [])]).
...
test(assrt_lib_abridged_notation) :-
    assrt_lib:assertion_records(pred check_to_messages(+Time      :ctime_t,
				...
				R,rt,[]),
    assertion(R=[assertions:assertion_db(check_to_messages(Time,
							   RTCheck,
							   Messages0,
							   Messages),
					 rt, check, pred,
