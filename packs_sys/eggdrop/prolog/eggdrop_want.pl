...
eggdropConnect(Host,Port,CtrlNick,Pass):-
  ...
  set_stream(OutStream,type(text)),set_stream(OutStream,encoding(octet)),
  set_stream(In,type(binary)),set_stream(In,encoding(octet)),
...
consultation_thread(CtrlNick,Port):- 
      ...
      to_egg('.echo off\n'),
      to_egg('.console ~w #logicmoo',[CtrlNick]),
      to_egg('.console ~w ""\n',[CtrlNick]),
...
:- dynamic(saved_err/1).
err_buff(Data):- assert(saved_err(Data)).
buffer_error(G):- with_error_to_buffer_predicate(line,err_buff,G),forall(retract(saved_err(D)),say(D)).
...
%% consultation_codes( ?CtrlNick, ?Port, ?Text) is det.
%
% Consultation Text.
%
...
irc_filtered(Channel,Agent,[S|TERM],Vs):- is_list([S|TERM]),is_lisp_call_functor(S),!,
   (current_predicate(lisp_call/3) -> irc_call(Channel,Agent,lisp_call([S|TERM],Vs,R),['Result'=R|Vs]);
     my_wdmsg(cant_irc_filtered(Channel,Agent,[S|TERM],Vs))).
irc_filtered(Channel,Agent,CALL,Vs):- lmconf:chat_isRegistered(Channel,Agent,executeAll),!,irc_call(Channel,Agent,CALL,Vs),!.
irc_filtered(Channel,Agent,CALL,Vs):- my_wdmsg(unused_irc_filtered(Channel,Agent,CALL,Vs)),!. 
...
with_error_to_output(CMD):- !,
   with_error_to_predicate(write,CMD).
...
%% to_egg( +Fmt, +Args) is det.
%
% Converted To Egg.
%
to_egg(Fmt,Args):-
   once(egg:stdio(_Agent,_InStream,OutStream)),
   ignore(catch(to_egg(OutStream,Fmt,Args),E,debugm(E==to_egg(OutStream,Fmt,Args)))),!.

to_egg(OutStream,Fmt,Args):-
   format(atom(S),Fmt,Args),
   debugm(writeln(OutStream,S)),
   set_stream(OutStream,type(text)),set_stream(OutStream,encoding(text)),
   writeln(OutStream,S),
...
% privmsg2(Channel,Text):- sleep(0.2),escape_quotes(Text,N), to_egg('.tcl putquick "PRIVMSG ~w :~w"',[Channel,N]),!.
privmsg2(Channel,Text):- sleep(0.2),escape_quotes(Text,N), to_egg('.msg ~w ~w',[Channel,N]),!.

%% putnotice( ?Channel, ?Text) is det.
%
% Putnotice.
%
putnotice(Channel,Text):- escape_quotes(Text,N),to_egg('.tcl putserv "NOTICE ~w :~w"',[Channel,N]).
