...
test :-
 maplist(abolish,[foo/1,foo/2]),
 maplist(abolish,[bar/2]),
 maplist(abolish,[qux/2]),
 maplist(dynamic,[foo/1,foo/2]),
 maplist(dynamic,[bar/1,bar/2]),
 maplist(dynamic,[qux/1,qux/2]),
 pfc_add_bc_rule(foo(X), (bar(X),qux(X))),
 pfc_add_bc_rule(bar(Y), bar(f(Y))),
%  pfc_add_bc_rule(bar(f(1)), iz_true),
 maplist(listing,[foo,bar,qux,iz_true]).
