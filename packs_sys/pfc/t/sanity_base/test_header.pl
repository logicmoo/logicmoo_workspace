...
:- endif. % \+ current_module(pfc_lib)

:- include(library(shared_test_header)).

:- if(assert_if_new((pfcops:pfc_op_decls((
...
:- if((current_prolog_flag(test_module,Module), pfcops:pfc_op_decls(OPS), call(Module:OPS))). :- endif.
...
:- if((current_prolog_flag(test_typein_module,Module), pfcops:pfc_op_decls(OPS), call(Module:OPS))). :- endif.

:- if((\+ current_module(pfc_test), ensure_loaded(library(pfc_test)))). :- endif.
...
:- expects_dialect(pfc).
:- else.
:- if((dmsg(this_test_might_need(:- use_module(library(pfc_syntax)))))).  :- endif.
:- endif.
