...
bridge_d(K,Ende):-
	...
	% special handling of disjunctions

handle_disj([]).
handle_disj([[A,B,C,D]|R]):-
    d(A,B,C,D),
    handle_disj(R).

          % near to cardinality
          % (c.f. S. Breitinger and H. Lock method)
          % encodes B in 0..1, X1+D1 #=<X2+256*B, X2+D2 #<=X1+256-256*B.

d(X1,D1,X2,D2):-
    domain(B,0,1), X1+D1 #=<X2+256*B, X2+D2 #=<X1+256-256*B. 
