...
% File SWI_GULP_ATSIGN.PL
% William R. Murray
% Teknowledge

% GULP 3.1 used by permission, this version is TEK GULP to refer to the
% changes made by Teknowledge. These are mostly minor changes so that feature
% operator is '<->' instead of ':' to provide
% compatibility with SWI-Prolog's use of the latter for modules, along
% with formatting and printing changes, and suppression of feature values
% that are equal to the atom 'empty'.
...
% The calls to the tracer, help utility, and apropos utility are
% performed entirely as a kludge to force loading these before
% --> is redefined in GULP. Otherwise these tools are not available
% for debugging after GULP is loaded as the redefinition of --> by
% GULP appears to interfere with their loading.

% If a way can be found to manually load these utilities before hand,
% that of course will be even better.

% :- format('Loading GUI Tracer code first before --> is redefined..~n').

% fact(0,1).

% fact(N,Factorial) :- N > 0,Sub1 is N - 1,fact(Sub1,M),Factorial is N * M.
...
g_version('GULP 3.1a for SWI-Prolog, 28 July 1994, mod. 17 September 2001 to use <-> instead of : for features--WRM.').
...
:- op(600, xfy, '<->').  % Changed from ':' to '<->' to prevent clashes with SWI Prolog's module prefix.
:- op(602, xfy, '..').
:- op(602, xfy, '::').
...
g_translate(Structure,List) :-
	...
	!,
	nonvar(Structure),
	g_translate_forward(Structure,List).

g_translate(Structure,List) :-
	nonvar(List),
	g_translate_backwards(Structure,List).
...
/*
 * g_translate_backwards(FeatureStructure,ValueList)     "Translate Backward"
 *
 *   Translates backward using g_backward_schema.
 */


g_translate_backwards(Value, Value) :-
	gulp__dont_translate(Value),
        !.

        /* Variables and atomic terms do not need any conversion. */

g_translate_backwards(FS,Term) :-
	\+ functor(Term,g__,_),
	!,
	Term =.. [Functor | Args],
	g_translate_backwards_list(NewArgs,Args),
	FS =.. [Functor | NewArgs].
...
g_translate_backwards(FS,Term) :-
	...
	g_translate_backwards_fixup(RawFS,FS).

        /* If we get here, we know Term is a value list. */


/*
 * g_translate_backwards_fixup(RawFeatureStructure,FeatureStructure)
 *
...
g_translate_backwards_fixup(F<->V,Result) :-                  /* Singleton case */
	g_translate_backwards_fixup_rest(F<->V,_,Result).

g_translate_backwards_fixup(F<->V..Rest,Result) :-
	g_translate_backwards(BTV,V),
	g_translate_backwards_add(F<->BTV,_,FV),
	g_translate_backwards_fixup_rest(Rest,FV,Result).   /* Start the recursion */

g_translate_backwards_fixup_rest(F<->V..Rest,ResultSoFar,Result) :-
	g_translate_backwards(BTV,V),
	g_translate_backwards_add(F<->BTV,ResultSoFar,FVR),
	g_translate_backwards_fixup_rest(Rest,FVR,Result).  /* Continue the recursion */

g_translate_backwards_fixup_rest(F<->V,ResultSoFar,FVR) :-
	g_translate_backwards(BTV,V),
	g_translate_backwards_add(F<->BTV,ResultSoFar,FVR).   /* End the recursion */


g_translate_backwards_add(_<->V,R,R)          :- var(V), !.   /* Unmentioned variable */
g_translate_backwards_add(F<->g_(V),R,F<->V)    :- var(R).      /* First contribution
                                                        to empty R */
g_translate_backwards_add(F<->g_(V),R,F<->V..R) :- nonvar(R).   /* Ordinary case */


/*
 * g_translate_backwards_list(FeatureStructureList,ValueListList)
 *
 *   Applies g_translate_backwards to ValueListList giving FeatureStructureList.
 */

g_translate_backwards_list([],[]).

g_translate_backwards_list([FH|FT],[VH|VT]) :-
	g_translate_backwards(FH,VH),
	g_translate_backwards_list(FT,VT).
...
/*
 * g_translate_forward(FeatureStructure,ValueList)     "Translate Forward"
 *
...
g_translate_forward(Term,Term) :-
	gulp__dont_translate(Term),
        !.

        /* Simplest and most frequent case: Term is atomic. */

g_translate_forward(Term,_) :-
	g_not_fs(Term),
	functor(Term,X,_),
	(X = '<->' ; X = '..' ; X = '::'),
	!,
...
g_translate_forward(Term,NewTerm) :-
	g_not_fs(Term),
	!,
	Term =.. [Functor|Args],
	g_translate_forward_list(Args,NewArgs),
...
g_translate_forward(Feature<->Value,ValueList) :-
	!,
	g_translate_forward(Value,NewValue),
	g_translate_forward_single_feature(Feature,g_(NewValue),ValueList).
...
g_translate_forward(FeatureStructure .. Rest,ValueList) :-
	!,
	g_translate_forward(FeatureStructure,VL1),
	g_translate_forward(Rest,VL2),
...
/*
 * g_translate_forward_list(ListOfTerms,ListOfResults)  "Translate Forward List"
 *
 *       Applies g_translate_forward to a list of arguments giving a list of results.
 */


g_translate_forward_list([],[]).

g_translate_forward_list([H|T],[NewH|NewT]) :-
	g_translate_forward(H,NewH),
	g_translate_forward_list(T,NewT).


/*
 * g_translate_forward_single_feature(Keyword,Value,ValueList)      "Translate Forward Single Feature"
 *
...
g_translate_forward_single_feature(Keyword,Value,ValueList) :-
...
display_feature_structure(Term) :-
	g_translate_backwards(FS,Term), /* Convert value lists into feature structures */
	g_display_indented(0,0,FS).  /* Display them */


/*
 * g_display_indented(CurPos,Indent,FS)     "Display Indented"
 *
...
g_display_indented(CurPos,Indent,Variable) :-
	var(Variable),
	!,
	g_display_indented_tab_over(Indent,CurPos),
	write(Variable),
	nl.

g_display_indented(_CurPos,Indent,_F<->empty..Rest) :-   /* WRM: Added this to skip over 'empty' values in feature lists */
	!,
	g_display_indented(_CurPos,Indent,Rest).         /* WRM: Changed from 0 back to _CurPos 6/18/02 */

g_display_indented(CurPos,Indent,F<->V..Rest) :-
	!,
	g_display_indented(CurPos,Indent,F<->V),
	g_display_indented(0,Indent,Rest).

g_display_indented(CurPos,Indent,F<->V::Rest) :-
	!,
	g_display_indented(CurPos,Indent,F<->V..Rest).  /* For compatibility */

g_display_indented(_CurPos,_Indent,_F<->empty) :-       /* WRM: Added this to skip over 'empty' values in feature lists */
	!.

/* WRM: Added this to start a new line when one feature structure is embedded in another */
g_display_indented(CurPos,Indent,F<->V) :-                 
	V = X<->Y..Rest,         
	!,
	g_display_indented_tab_over(Indent,CurPos),
	...
	g_display_indented(0,NewIndent,V).
...
g_display_indented(CurPos,Indent,F<->V) :-
	...
	write(F), write('<->'),
	g_printlength(F,PL),
	NewIndent is Indent+PL+3,        /* WRM: Changed from +2 to +3 as '<->' is 3 chars long, not the two char '::' it replaces */
	g_display_indented(NewIndent,NewIndent,V).

g_display_indented(CurPos,Indent,OrdinaryTerm) :-
	g_display_indented_tab_over(Indent,CurPos),
	write(OrdinaryTerm),
	nl.
...
/**************************************
 * Tek GULP Extensions to GULP I/O -- WRM
 **************************************/

% display_feature_structure(Features,Output) is just
% like display_feature_structure/1 but returns its output
% in the string Output, rather than printing it out -- WRM

% Used in the CELT server. 

display_feature_structure(Term,Output) :-
	g_translate_backwards(FS,Term),        /* Convert value lists into feature structures */
	g_display_indented(0,0,FS,Output).  /* Display them */

/*
 * g_display_indented(CurPos,Indent,FeatureStructure,Output)     "Display Indented"
 *
 *   CurPos is the current position on the line;
 *   Indent is the indentation at which this item should be printed.
 *   FeatureStructure is the feature structure
 *   Output is the final string output
 */

% This could be made more efficient by changing the order of
% arguments so that indexing on the first argument would work.

g_display_indented(CurPos,Indent,Variable,Output) :-
	var(Variable),
	!,
	g_display_indented_tab_over(Indent,CurPos,Tab_Output),
	sformat(Output,'~s~s~n',[Tab_Output,Variable]).

g_display_indented(_CurPos,Indent,_F<->empty..Rest,Output) :-   /* WRM: Added this to skip over 'empty' values in feature lists */
	!,
	g_display_indented(_CurPos,Indent,Rest,Output).         /* WRM: Changed from 0 back to _CurPos 6/18/02 */

g_display_indented(CurPos,Indent,F<->V..Rest,Output) :-
	!,
	g_display_indented(CurPos,Indent,F<->V,First_Output),
	g_display_indented(0,Indent,Rest,Rest_Output),
	sformat(Output,'~s~s',[First_Output,Rest_Output]).

g_display_indented(CurPos,Indent,F<->V::Rest,Output) :-
	!,
	g_display_indented(CurPos,Indent,F<->V..Rest,Output).  /* For compatibility */

g_display_indented(_CurPos,_Indent,_F<->empty,Output) :-       /* WRM: Added this to skip over 'empty' values in feature lists */
	!,
	sformat(Output,'').                                    /* WRM: Suppressed extra empty line here */

/* WRM: Added this to start a new line when one feature structure is embedded in another */
g_display_indented(CurPos,Indent,F<->V,Output) :- 
	V = X<->Y..Rest,         
	!,
	g_display_indented_tab_over(Indent,CurPos,Tab_Output),
	sformat(First_Output,'~a<->~n',[F]),      /* WRM: F should be an atom here */
	g_printlength(F,PL),
	NewIndent is Indent+PL+3,
	g_display_indented(0,NewIndent,V,Final_Output),
	sformat(Output,'~s~s~s',[Tab_Output,First_Output,Final_Output]).

g_display_indented(CurPos,Indent,F<->V,Output) :-
	!,
	g_display_indented_tab_over(Indent,CurPos,Tab_Output),
	sformat(First_Output,'~a<->',[F]),      /* WRM: F should be an atom here */
	g_printlength(F,PL),
	NewIndent is Indent+PL+3,        /* WRM: Changed from +2 to +3 as '<->' is 3 chars long, not the two char '::' it replaces */
	g_display_indented(NewIndent,NewIndent,V,Final_Output),
	sformat(Output,'~s~s~s',[Tab_Output,First_Output,Final_Output]).

g_display_indented(CurPos,Indent,OrdinaryTerm,Output) :-
	g_display_indented_tab_over(Indent,CurPos,Tab_Output),
	sformat(Output,'~s~w~n',[Tab_Output,OrdinaryTerm]).

g_display_indented_tab_over(Indent,CurPos,Output) :-
	Tabs is Indent-CurPos,
	sformat(Output,'~*c',[Tabs,32]).

/* WRM: Changed last line to take numeric argument from Tabs then write out char code 32 (space) that many times. Equivalent
to tab(Tabs), which it replaces. */
...
writeln(X) :- g_translate_backwards(TranslatedX,X), writeln_aux(TranslatedX).
...
portray(X) :- g_vl(X), g_translate_backwards(FS,X), write(FS).   %%  Quintus, LPA, and SWI
...
%%  Other Prologs (default for a virgin file)
%%
%%/***************************************************************************
