...
set_pos(DPos,OL):- (member(pos(OLD),OL) -> ignore((OLD\==DPos, remove_el_via_setarg(OL,pos(OLD)), nb_set_add(OL,old_pos(OLD)),nb_set_add(OL,pos(DPos))));nb_set_add(OL,pos(DPos))).
set_pos(N,DPos,OL):- (member(pos(OLD),OL) -> ignore((sub_atom(DPos,0,N,_,LP),sub_atom(OLD,0,N,_,RP), 
  (LP\==RP ;( atom_length(DPos,L1),atom_length(OLD,L2), L1>L2)),
  nb_set_add(OL,old_pos(OLD)),nb_set_add(OL,pos(DPos)),
  ignore(remove_el_via_setarg(OL,pos(OLD))))); nb_set_add(OL,pos(DPos))).
...
add_spans_to_w2(RW2s,Span,RW2so):- Span = span(List),
  ...
  nb_set_add1(W2,Span),
  RW2so = RW2s,!. 
add_spans_to_w2(RW2s,span(List),RW2so):- member(span(_),List),add_spans_to_w2(RW2s,List,RW2so),!.
