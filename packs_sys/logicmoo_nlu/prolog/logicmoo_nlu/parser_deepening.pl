...
%if_search_expanded(N,_):- N>5, !, fail.
...
debug_chat80_if_fail(MG):-  strip_module(MG,M,G), locally(set_prolog_flag(no_pretty,true),debug_chat80_if_fail(M,G)).
