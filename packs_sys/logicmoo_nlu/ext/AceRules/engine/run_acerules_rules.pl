...
% Quick and dirty hack to just output the rules generated by Ace Rules.
...
process(In, Out, Semantics, normal) :-
	...
	read_stream_to_codes(In, Codes),
		...
		(
			generate_output_rules(Codes, Semantics, [maxanswers=10], Rules),
%			simple_rules(Rules, SimpleRules),
			print_normal(Out, Rules)
		),
