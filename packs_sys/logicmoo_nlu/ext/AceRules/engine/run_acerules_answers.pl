% Modification of run_acerules by Martin D.
% Quick and dirty hack to just run the Ace Rules verbaliser.
...
run(InFile, InAFile, OutFile) :-
    run(InFile, InAFile, OutFile, normal).
...
run(InFile, InAFile, OutFile, Mode) :-
    open(InFile, read, In),
    open(InAFile,read, InA),
    open(OutFile, write, Out),
    init_ulex,
    process(In, InA, Out, Mode),
    close(In),
    close(Out),
...
process(In, InA, Out, normal) :-
    no_debug,
	 read_stream_to_codes(In, Codes),
      readData(InA,Answerset),
  %   write_terms(Out, Answersets),
% close(In),
%  show_records(Answersets),
        catch(
		(
		        generate_output_answers(Codes, Semantics, [maxanswers=10], Rules, [AnswerSet], AnswerTexts),
			simple_rules(Rules, SimpleRules),
		%        print_normal(Out, [Answerset], AnswerTexts)
			print_normal(Out, Codes, Rules, SimpleRules, [Answerset], AnswerTexts)
		),
		error(_, ErrorMessage),
		format(Out, 'ERROR: ~w\n', ErrorMessage)
	).

process(In, InA, Out, debug) :-
    debug,
	read_stream_to_codes(In, Codes),
        readData(InA,Answerset),
	catch(
