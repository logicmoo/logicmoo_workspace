...
:- if(exists_source(pldata('kb_7166ss.pl-a3'))).
:- include(pldata('kb_7166ss.pl-a3')).
:- endif.
...
:- set_prolog_flag(kb7166ss:double_quotes,string).
:- (compiling->ra5;true).
...
setup7166ss:- call(retractall,backward(_,'[]',_)),call(retractall,code(_,'[]',_)),call(retractall, forward(_,'[]',_)).
