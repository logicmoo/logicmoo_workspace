...
resource_of_type( domain_travel, domain ).
resource_of_type( domain_autoroute, domain ).

resource_relation( relevant, [Domain, Answer, Query ] ) :-
	relevant_answer( Query, Answer, Domain ).
resource_relation_type( relevant_answer, [domain, _, question ] ).

resource_relation( resolves, [Domain, Answer, Query ] ) :-
	resolves( Query, Answer, Domain ).
resource_relation_type( resolves, [domain, _,  question] ).
...
%resource_relation( consultDB, [_, _X^Q, _, fail(Q) ] ).
