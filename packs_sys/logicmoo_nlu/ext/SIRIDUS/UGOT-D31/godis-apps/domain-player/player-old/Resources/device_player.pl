...
%%% 	oaag:solve(whatIsPlaying(_S),[],S), %%whatIsPlaying/1
%%% 	( S = [] -> Tit = '', Grp = '' ;
%%% 	    fix_list(S,Tit-Grp)
%%% 	).

perform_query( X^current_song(X), current_song([Tit,Grp]) ):-
	oaag:solve(whatIsPlaying(S)), %%whatIsPlaying/1
	( S = [] -> Tit = '', Grp = '' ;
	    fix_list(S,Tit-Grp)
	).
...
%%% 	oaag:solve(openPlayerMenu).

%%% perform_command( 'OpenSubstartMenu' ):-
%%% 	oaag:solve(openSubstartMenu).

%%% perform_command( 'OpenPlaylistMenu' ):-
%%% 	oaag:solve(openPlaylistMenu).
...
perform_command( 'Start' ):-
	...
	dev_set( player_status, playing ),
 	oaag:solve(playPlayer).
%%% %%	format("tar bort start igen...~w\n",[A]),
%%% 	oaag:solve(deletePlan('Start')).
...
perform_command( 'StartSpecific' ):-
	...
	;
	  ToPlay = [föregående],
	  oaag:solve(playPrevious)
	;
	  fix_item(ToPlay,Ind),
	  Index is Ind - 1,
	  oaag:solve(playPlayer(Index))
	)
	;
	dev_get( what_to_play, ToPlay ),
	(
...
perform_command( 'PlaylistDelSpecific' ):- %% remItems
	...
	Index is Ind2 - 1,
	oaag:solve(removeIndex(Index)).
	
perform_command( 'PlaylistShuffle' ):-
	oaag:solve(shuffleList(_A)). 
...
perform_command( 'IncreaseVol' ):-
	oaag:solve(setGain(0.9)).
perform_command( 'DecreaseVol' ):-
	oaag:solve(setGain(0.3)).

perform_command( 'ShowList' ):-
	oaag:solve(showList(List)),
	fix_list(List,[],Ans),
	print_ans(1,Ans),
	format("\n",[]).
