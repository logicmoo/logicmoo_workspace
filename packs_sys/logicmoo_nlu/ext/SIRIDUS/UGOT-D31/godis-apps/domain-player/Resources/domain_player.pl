...
postcond( handle_player, done(fast_rewind) ).
...
postcond( fast_rewind, done(fast_forward) ).
postcond( fast_rewind, done(rewind) ).
postcond( fast_rewind, done(next_song) ).
postcond( fast_rewind, done(previous_song) ).
...
%	dev_queryAll(dbase, X^albums_by_artist(X) )
%      ]).
...
%	dev_queryAll(dbase, Group^artists_song(Group))
%      ]).
...
%	dev_queryAll(dbase, Art^artists_album(Art) ),
...
plan( Songs^songs_by_artist(Songs),
	...
	findout(Art^group(Art)),
	dev_queryAll(dbase,Songs^songs_by_artist(Songs))
      ]).
...
sort_restr( itemRem(X) ):-       sem_sort(X,index).
sort_restr( groupToAdd(X) ):-    sem_sort(X, group).
sort_restr( artist(X) ):-       sem_sort(X, group).
sort_restr( group(X) ):-         sem_sort(X, group).
sort_restr( song_artist(X) ):-   sem_sort(X, group).
sort_restr( album(X) ):-         sem_sort(X, album ).
sort_restr( artists_song(X) ):-  sem_sort(X, group).
sort_restr( artists_album(X) ):- sem_sort(X, group).
sort_restr( albums_by_artist(X) ):- sem_sort(X, album ).
sort_restr( songs_by_artist(X) ):-  sem_sort(X, item).
sort_restr( station(X) ):-	 sem_sort(X, station).
%sort_restr( year(X) ):-          sem_sort( X, year ).
sort_restr( path(X) ):-          atomic( X ).%,format("hallå: ~w\n",[X]).
%sort_restr( X^path(X) ):-          atomic( X ),format("hallå: ~w\n",[X]).
sort_restr( not path(X) ):-      atomic( X ).
sort_restr( fail(Path^path(Path),no_matches) ).
sort_restr( fail(_Q^_Query,no_matches) ).
%% "ren" databassökning
% issues
sort_restr( issue(Q) ):-
	plan( Q, _ ),
