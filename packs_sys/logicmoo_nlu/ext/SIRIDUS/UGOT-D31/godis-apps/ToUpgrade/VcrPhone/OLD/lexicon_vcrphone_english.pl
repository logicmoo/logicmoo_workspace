...
:- module( lexicon_vcrphone_english, [output_form/2, input_form/2, input_form2/3,
				      digit_word/2, yn_answer/1, time/1]).
...
output_form( ask(X^new_program_position(X)),
	     "What program position do you want?" ).
...
output_form( task(vcr_top), "go the VCR manager" ).
...
output_form( task(tp_top), "go the telephone manager" ).
output_form( task(tp_base_station), "go the base station" ).
...
output_form( inform(now_playing), "OK. The VCR is now playing." ).
output_form( inform(already_stopped), "The VCR is already stopped." ).
output_form( inform(now_stopped), "OK. The VCR is now stopped." ).
output_form( inform(already_ff), "The VCR is already fast-forwarding." ).
output_form( inform(now_ff), "OK. The VCR is now fast-forwarding." ).
output_form( inform(already_rewinding), "The VCR is already rewinding." ).
output_form( inform(now_rewinding), "OK. The VCR is now rewinding." ).
output_form( inform(already_paused), "The VCR is already paused." ).
output_form( inform(now_paused), "OK. The VCR is now paused." ).
output_form( inform(already_recording), "The VCR is already recording." ).
output_form( inform(now_recording), "OK. The VCR is now recording." ).
...
output_form( inform(clock_was_set), "OK. The clock was set." ).
...
output_form( inform(programs(Programs)), Cs ) :-
	...
	append( "Which program number to you want to delete?\n", Listing, Cs ).

output_form( inform(no_program_to_delete), "There is no program to delete." ).
output_form( inform(program_deleted), "OK. The program was deleted." ).
...
output_form( inform(answering_machine_now_on),
	     "OK. The answering machine is now switched on." ).
...
output_form( inform(signal_was_set(SignalType,ToneOrMelody)), Cs ) :-
	...
	format_to_chars('OK. ~s calls will use ~s.',[SignalTypeCs,ToneOrMelodyCs],Cs).
...
output_form( reqRep(understanding), "I didnt understand what you said. Please rephrase." ).
...
input_form( [basic],                answer(task(tp_base_station_settings_basic)) ).
...
input_form( [search],               answer(signal_type(search_signal)) ) .
...
input_form2( [from] + answer(number(T)), answer(start_time_to_store(T)), time(T) ).

input_form2( [to] + answer(number(T)), answer(stop_time_to_store(T)), time(T) ).

input_form2( [switch,to,[program,channel]] + answer(number(P)), answer(new_program_position(P)), true ).

input_form2( [record,[program,channel]] + answer(number(P)), answer(program_position_to_store(P)), true ).

input_form2( [[program,channel]] + answer(number(P)), answer(program_position(P)), true ).

input_form2( [delete,program,maybe(number)] + answer(number(N)), answer(program_number(N)), true ).

input_form2( [clock,to] + answer(number(C)), answer(new_clock(C)), true ).

input_form2( [add] + answer(name(N)), answer(phonebook_name_to_add(N)), true ).

input_form2( [delete] + answer(name(N)), answer(phonebook_entry_to_delete(N)), true ).

input_form2( [[call,dial]] + answer(name(N)),
	     [ answer(call(_)), answer(phonebook_name_to_find(N)) ], true ).
