...
/*************************************************************************
         ...
         name: indites.pl 
      version: 
  description: InDiTeS specification file
...
:- assertz(user:library_directory(home('Jobb/TRINDI/TRINDIKIT/trindikit1.1'))).
:- assertz(user:library_directory(home('Jobb/TRINDI/TRINDIKIT/trindikit1.1/Datatypes'))).
:- assertz(user:library_directory(home('Jobb/TRINDI/TRINDIKIT/trindikit1.1/Modules'))).
:- assertz(user:library_directory(home('Jobb/TRINDI/InDiTeS'))).
...
selected_datatypes([standard, record, set, stack, stackset, assocset, indites_datatypes]).
...
selected_modules([ control: controlT,
...
selected_macro_file( indites_macros ).
...
reset_operations( [ set(program_state, run),
		    ...
		    % add#rec(shared^com,(task(instruct_exec(reinstall(print_head))))),
		    push#rec(private^agenda, inform(task(instruct_exec(reinstall(print_head)))))]):-
	flag( language, Lang ),
	flag( domain, Domain ).
