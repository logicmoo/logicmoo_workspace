...
%	member( P:_, Programs ),
%	!.
dev_query( program_exists(PA), program_exists(PA) ) :-
	dev_get( programs, Programs ),
	to_number( PA, P ),
	member( P:_, Programs ),
	!.

dev_query( program_exists(PA), not program_exists(PA) ) :- !.

dev_query( available_program_slot, not available_program_slot ) :-
	dev_get( programs, Programs ),
	length( Programs, 8 ),
	!.

dev_query( available_program_slot, available_program_slot ).


dev_query( play_status(S), play_status(S) ):-
	dev_get( play_status, S ),!.

dev_query( play_status(S), not play_status(S) ).
