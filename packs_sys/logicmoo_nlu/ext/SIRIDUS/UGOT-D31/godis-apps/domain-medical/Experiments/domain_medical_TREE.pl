...
plan( X^disease(X),
      [ raise(X^symptom(X)),
	findout(symptom(headache)),
	if_then_else( symptom(headache),
		      [ findout( symptom(fever) ),
			if_then_else( symptom(fever),
				      assume(disease(malaria)),
				      assume(disease(none)) ),
			report( take_test(fever), pending ),
			assume_issue( test_result(fever) ),
			dev_do( medical, take_test(fever) ),
			dev_query( medical, test_result(fever) ),
			if_then( test_result(fever),
				 assume(disease(asschychotostocsusis)) ) ],
		      assume( disease(none) ) ),
	bind( X4^symptom(X4) ),
...
plan( take_test,
	...
	if_then( test(T),
		 [ % activate nurse-device to take the test
		   dev_do( nurse, take_test(T) ),
		   % get results of test
		   dev_query( nurse, X6^test_result(X6) ),
*/
		   
/*
plan( X1^disease(X1),
      [ %if_then_else( symptom(_), [], [raise( X8^symptom(X8) )] ),
	dev_query( medical, X2^bestq(X2) ),
	if_then_else( bestq(Q),
		      [ findout(Q),
			forget( bestq(Q) ) ],
		      [ dev_query( medical, X9^best_labtest(X9) ),
			if_then( best_labtest(T), % T is y/n question
				 [ report( take_test(T), pending ),
				   % assume_issue( X10^result(X10) ),
				   dev_do( medical, take_test(T) ),
				   dev_query( medical, X6^test_result(X6) ), %dev_query( nurse, T ),
				   forget( status(T, pending ) ),
				   forget( best_labtest(T) ) ] ) ] ),	
...
/*plan( X^info(X),
	...
	if_then(info_disease(D),
		assume(info(D)))]).
*/
%	dev_query( medical, X^info(X) ) ] ).


%infer( [ traveled_to( C ), tropical( C ) ], [ traveled_to_tropical ] ).
...
/*
plan( X^disease(X),
      [ dev_query( medical, X1^bestq(X1) ),
	if_then_else( bestq(Q),
		      [ findout(Q),
			forget( bestq(Q) ) ],
		      if_then( besttest(T), % T is y/n question
			       [ dev_query( nurse, T ),
				 forget(besttest(T) ) ] ) ),	
	dev_query( medical, X3^disease(X3)),
	bind( X2^symptom(X2) ) ] ).
	...
	if_then_else( symptom(headache),
		    [ findout( symptom(fever) ),
		      if_then_else( symptom(fever),
				    assume(disease(malaria)),
				    assume(disease(none)) ) ],
%				    [ dev_do( nurse, check_fever ),
%				      dev_query( nurse, lab_result() ) ),
%				      if_then(temp(T) and T>38, assume(fever) )] ) ],
		    assume(disease(asschychotostocsusis)) ),
	bind(symptom(fever)),
	bind(symptom(headache))] ).
*/		  
