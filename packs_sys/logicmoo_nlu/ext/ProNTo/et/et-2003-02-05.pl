% et.pl - M. Covington      2003 February 5
...
%  it as specified in char_type_char.  If it's a "null"
%  character, it is skipped and the next character is read.

get_char_and_type(Stream,Char,Type) :-
   get_char(Stream,C),
   char_type_char(C,Type1,Char1),
   get_char_and_type_x(Stream,Char1,Type1,Char,Type).

get_char_and_type_x(Stream,_,null,Char,Type) :-
   % skip null characters
   !,
   get_char_and_type(Stream,Char,Type).

get_char_and_type_x(_,Char,Type,Char,Type).
   % not a null character, so just return it
...
% Null characters
% (these are simply ignored, as if they were not in the input)
char_table('''',  null,  '''').
