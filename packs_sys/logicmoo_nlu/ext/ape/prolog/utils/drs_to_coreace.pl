...
drs_to_coreace(DRS0, AceSentenceList) :- unnumbervars(DRS0,DRS),
	...
	trace_failure(drs_to_coreace:get_toplevel_object_referents(ConditionList, toplevel(ToplevelReferentList, UnsortedSubjectList, UnsortedObjectList, NamedList))),
	ape_numbervars(DRSCopy, 0, _),

	trace_failure(drs_to_coreace:drs_to_coreace_nvar_wrapper(drs(Dom, ConditionList), [ToplevelReferentList, UnsortedSubjectList, UnsortedObjectList, NamedList], Result)),
	!,
	trace_failure(drs_to_coreace:post_process(Result, AceSentenceList)),
	!.
