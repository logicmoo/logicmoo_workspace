...
%% get_owl_output(+OwlOutputType:atom, +Comment:atom, +Drs:term, +Uri:atom, -ContentType:atom, -PrologTerm:term,  -Output:atom) is det.
%
...
%
get_owl_output(owlrdf, Comment, Drs, Uri, 'text/xml', PrologTerm, Output) :-
	...
	;
        ...
        PrologTerm = owlfss_owlrdfxml(Comment, OwlFss, OwlRdfxml).

get_owl_output(owlfss, Comment, Drs, Uri, 'text/plain', PrologTerm, Output) :-
	ignore(drs_to_owlswrl:drs_to_owlswrl(Drs, Uri, Comment, OwlFss)),
	(
		get_messages_with_type(owl, [])
	->
	...
	;
        ...
        PrologTerm = e(Comment,OwlFss).

get_owl_output(owlfsspp, Comment, Drs, Uri, 'text/plain', PrologTerm, Output) :-
	ignore(drs_to_owlswrl:drs_to_owlswrl(Drs, Uri, Comment, OwlFss)),
	(
		get_messages_with_type(owl, [])
	->
...
get_owl_output(owlxml, Comment, Drs, Uri, 'text/xml', PrologTerm, Output) :-
	ignore(drs_to_owlswrl:drs_to_owlswrl(Drs, Uri, Comment, OwlFss)),
	(
		get_messages_with_type(owl, [])
	->
	...
	;
		Output = ''
	),PrologTerm = drs_to_owlswrl(Comment,OwlFss,OwlXml).
