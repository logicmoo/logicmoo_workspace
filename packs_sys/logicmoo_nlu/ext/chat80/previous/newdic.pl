...
:- only_pfc(retractall(nldata_BRN_WSJ_LEXICON:text_bpos(the,nn))).

% :- begin_dynamic_reader.
:- asserta((t_l:enable_src_loop_checking)).


deduce_subj_obj_LF(PropAttrib,Adj,Type,X,TypeY,Y,H):-
  subj_obj_LF(PropAttrib,Adj,Type,X,TypeY,Y,H).
...
parser_chat80:txt_no_db(not,not).
txt_no_db(no,no).
...
subject_LF(thing,Type,TYPEMASK,X,isa(X,Type)):- plt,loop_check_u(tSet(Type)),atom(Type),gen_typemask(Type,TYPEMASK).
subject_LF(restriction,Type,TYPEMASK,X,isa(X,Type)):- plt,loop_check_u(tSet(Type)),atom(Type),gen_typemask(Type,TYPEMASK).


gen_typemask(Type,measure&Type&_):- call_u(ttFormatType(Type)),!.
gen_typemask(Type,feature&Type&_).
gen_typemask(_,feature&_).


noun_plu_db_via_types(TS,T):- maybe_noun_or_adj(T),maybe_noun_or_adj(TS), (atom(TS)->atom_concat(T,'s',TS);true),loop_check_u(tSet(T)),atom(T),atom_concat(T,'s',TS).
...
hasPropertyOrValueISA(T,PorV):- call_u(isa(PorV,T)).

hasProperty(Type,P):-no_repeats((Type-P),hasPropertyValueSVO(Type,P,_)).

hasPropertyValueSVO(SomeType,P,SomeVType):-call_u(mpred_arity(P,A)),A>=2, cycQuery80(argIsa(P,1,SomeType)),cycQuery80(argIsa(P,A,SomeVType)).
...
get_1st_order_functor(Pred,P):- \+ (compound(Pred)),!,P=Pred.
get_1st_order_functor(Pred,P):-get_functor(Pred,F),(is_2nd_order_holds_maybe(F)->((arg(1,Pred,A),!,get_1st_order_functor(A,P)));P=F).

:- if(current_module(pfc)).
is_2nd_order_holds_maybe(F):- is_2nd_order_holds(F).
:- else.
is_2nd_order_holds_maybe(_F):- fail.
:- endif.
...
typeAssignableTo(Type,SomeType):- call_u(transitive_subclass(Type,SomeType)).
typeAssignableTo(_Type,SomeType):- call_u(ttFormatType(SomeType)).
...
%name_db([Name],Name) :- t_l:useAltPOS,downcase_atom(Name,DCName),loop_check( \+ (cw_db(DCName,_))).
...
name_template_db(X,feature& ISA) :- plt,  nonvar(ISA), call_u(isa(X,ISA)).
name_template_db(X,feature& ISA & _) :- plt,  nonvar(ISA), call_u(isa(X,ISA)).
name_template_db(X,feature& _ & ISA ) :- plt,  nonvar(ISA), call_u(isa(X,ISA)).
...
ccw_db1(W,'Preposition'):-prep_db(W), \+ (ccw_db1(W,'Determiner')).
...
ccw_db3(W,C):-ccw_db4(W,C), \+ (loop_check(adj_db(W,_))),not_ccw(W).
...
cw_db1(W,C):-ocw_db(W,C),dif(C,OC), \+ (ocw_db(W,OC)).
cw_db2(W,C):-one_must(ocw_db0(W,C),ocw_db1(W,C)).

ocw_db(W,'Verb'):-no_repeats(W,(verb_form_db(W,_,_,_), \+ (ccw_db(W,CCW)),CCW\=W)).
ocw_db(W,C):-ocw_db0(W,C).

ocw_db0(W,C):-one_must(ocw_db1(W,C),ocw_db3(W,C)).
ocw_db1(W,'Interjection'):-talk_db(interj,W).
ocw_db1(W,'Adverb'):-adverb_db(W),\+ ocw_db2(W,_).
...
simplePOS(POS,SIMP):-call_u(posName(SIMP)),atom_concat(_,SIMP,POS).


subject_LF(restriction,AdjNounEan,feature&_,X,adjIsa(X,AdjNounEan)):- plt, adj_db(AdjNounEan,restr),not_ccw(AdjNounEan).
adjIsa(E,C):- only_pfc(call_u(isa_backchaing(E,C))),nonvar(C).
adj_db(AdjNounEan,restr):- plt,talk_db(adj,AdjNounEan),talk_db(noun1,AdjNounEan,_).
adj_db(AdjRestr,restr):-plt,talk_db(adj,AdjRestr), \+ (adj_db(AdjRestr,quant)), \+ (adverb_db(AdjRestr)).
...
subject_LF(thing,CountNoun,feature&_,X,isa(X,RootNoun)):-  plt, clex_noun(CountNoun, RootNoun,_,_,_), \+ (ratio_db(RootNoun,_,_,_)).
subject_LF(thing,CountNoun,feature&_,X,adjIsa2(X,RootNoun)):- plt, clex_adj(CountNoun, RootNoun,_), \+ (ratio_db(RootNoun,_,_,_)).
...
must_test_801([which, are, the, largest, african, countries, ?], [ parse(whq(feature&place&country-B, s(np(3+sg, wh(feature&place&country-B), []), verb(be, active, pres+fin, [], pos(_)), [varg(dir, np(3+sg, np_head(det(the(sg)), [sup(most, adj(large)), adj(african)], country), []))], []))), sem((answer80([A]):-B^ (setof(C:D, (country(D), area(D, C), african(D)), B), aggregate80(max, B, A)))), qplan((answer80([D]):-C^ (setof(B:A, (african(A), {country(A)}, area(A, B)), C), aggregate80(max, C, D)))), 
answers([sudan])],[time(0.0)]).
must_test_801([what, rivers, are, there, ?], [sent([what, rivers, are, there, ?]), parse(whq(feature&river-B, s(np(3+pl, np_head(int_det(feature&river-B), [], river), []), verb(be, active, pres+fin, [], pos(_)), [void(_Why)], []))), sem((answer80([A]):-river(A), A^true)), qplan((answer80([B]):-river(B), B^true)), 
...
must_test_801([does, afghanistan, border, china, ?], [sent([does, afghanistan, border, china, ?]), parse(q(s(np(3+sg, nameOf(afghanistan), []), verb(border, active, pres+fin, [], pos(_)), [varg(dir, np(3+sg, name(china), []))], []))), sem((answer80([]):-borders(afghanistan, china))), qplan((answer80([]):-{borders(afghanistan, china)})), 
answers([true])],[time(0.0)]).
must_test_801([what, is, the, capital, of, upper_volta, ?], [sent([what, is, the, capital, of, upper_volta, ?]), parse(whq(feature&city-B, s(np(3+sg, wh(feature&city-B), []), verb(be, active, pres+fin, [], pos(_)), [varg(dir, np(3+sg, np_head(det(the(sg)), [], capital), [pp(prep(of), np(3+sg, name(upper_volta), []))]))], []))), sem((answer80([A]):-capital(upper_volta, A))), qplan((answer80([A]):-capital(upper_volta, A))), 
answers([ouagadougou])],[time(0.0010000000000000009)]).
must_test_801([where, is, the, largest, country, ?], [sent([where, is, the, largest, country, ?]), parse(whq(feature&place&A-B, s(np(3+sg, np_head(det(the(sg)), [sup(most, adj(large))], country), []), verb(be, active, pres+fin, [], pos(_)), [varg(pred, pp(prep(in), np(_, np_head(int_det(feature&place&A-B), [], place), [])))], []))), sem((answer80([A]):-B^ (C^ (setof(D:E, (country(E), area(E, D)), C), aggregate80(max, C, B)), place(A), in_ploc(B, A)))), qplan((answer80([F]):-E^D^ (setof(C:B, (country(B), area(B, C)), D), aggregate80(max, D, E), in_ploc(E, F), {place(F)}))), 
answers([asia, northern_asia])],[time(0.0009999999999999731)]).
must_test_801([which, countries, are, european, ?], [sent([which, countries, are, european, ?]), parse(whq(feature&place&country-B, s(np(3+pl, np_head(int_det(feature&place&country-B), [], country), []), verb(be, active, pres+fin, [], pos(_)), [varg(pred, adj(european))], []))), sem((answer80([A]):-country(A), european(A))), qplan((answer80([A]):-european(A), {country(A)})), 
...
must_test_801([which, is, the, largest, african, country, ?], [sent([which, is, the, largest, african, country, ?]), parse(whq(feature&place&country-B, s(np(3+sg, wh(feature&place&country-B), []), verb(be, active, pres+fin, [], pos(_)), [varg(dir, np(3+sg, np_head(det(the(sg)), [sup(most, adj(large)), adj(african)], country), []))], []))), sem((answer80([A]):-B^ (setof(C:D, (country(D), area(D, C), african(D)), B), aggregate80(max, B, A)))), qplan((answer80([D]):-C^ (setof(B:A, (african(A), {country(A)}, area(A, B)), C), aggregate80(max, C, D)))), 
answers([sudan])],[time(0.0)]).
must_test_801([what, is, the, ocean, that, borders, african, countries, and, that, borders, asian, countries, ?], [sent([what, is, the, ocean, that, borders, african, countries, and, that, borders, asian, countries, ?]), parse(whq(feature&place&seamass-B, s(np(3+sg, wh(feature&place&seamass-B), []), verb(be, active, pres+fin, [], pos(_)), [varg(dir, np(3+sg, np_head(det(the(sg)), [], ocean), [conj(and, rel(feature&place&seamass-C, s(np(3+sg, wh(feature&place&seamass-C), []), verb(border, active, pres+fin, [], pos(_)), [varg(dir, np(3+pl, np_head(generic, [adj(african)], country), []))], [])), rel(feature&place&seamass-C, s(np(3+sg, wh(feature&place&seamass-C), []), verb(border, active, pres+fin, [], pos(_)), [varg(dir, np(3+pl, np_head(generic, [adj(asian)], country), []))], [])))]))], []))), sem((answer80([A]):-ocean(A), B^ (country(B), african(B), borders(A, B)), C^ (country(C), asian(C), borders(A, C)))), qplan((answer80([A]):-B^C^ (ocean(A), {borders(A, B), {african(B)}, {country(B)}}, {borders(A, C), {asian(C)}, {country(C)}}))), 
answers([indian_ocean])],[time(0.0020000000000000018)]).
must_test_801([what, are, the, capitals, of, the, countries, bordering, the, baltic, ?], [sent([what, are, the, capitals, of, the, countries, bordering, the, baltic, ?]), parse(whq(feature&city-B, s(np(3+pl, wh(feature&city-B), []), verb(be, active, pres+fin, [], pos(_)), [varg(dir, np(3+pl, np_head(det(the(pl)), [], capital), [pp(prep(of), np(3+pl, np_head(det(the(pl)), [], country), [reduced_rel(feature&place&country-D, s(np(3+pl, wh(feature&place&country-D), []), verb(border, active, inf, [prog], pos(_)), [varg(dir, np(3+sg, name(baltic), []))], []))]))]))], []))), sem((answer80([D]):-setof([A]:C, (country(A), borders(A, baltic), setof(B, capital(A, B), C)), D))), qplan((answer80([H]):-setof([E]:G, (country(E), borders(E, baltic), setof(F, capital(E, F), G)), H))), 
