...
(force-print ";; start loading cynd/cogbot-ke.lisp")
;;;;==================================================
;;;; MICROTHEORY SETUP
;;;;==================================================
(in-package "CYC")
...
(ke-assert-now `(#$isa ,*VocabularyMt*  #$VocabularyMicrotheory) *UVMt*)
...
(cbnl-assert `(#$isa ,(find-or-create-constant "stringSentenceList") #$BinaryPredicate) #$UniversalVocabularyMt)
...
(ke-assert-now `(#$implies (#$isa ?TEXT #$CharacterString)
...
(find-or-create-constant "nthInList")
...
(cbnl-assert `(#$isa ,(find-or-create-constant "subSentenceOfAIS") #$Predicate) #$UniversalVocabularyMt)
...
(ke-assert-now '(#$afterAdding #$originalPhrase 
...
(define-public intern-originalPhrase (assertion)   
 (clet ((formula  (assertion-formula assertion))
	...
	(cdolist (var (cdr (split-string-sentences string)))
	  ...
	  (print sent)
	  (ke-assert-now sent mt)))
	  ))

  

 ;; (ke-assert-now '(#$originalPhrase #$L2R-SentenceLevelPeg-2 "The Ap dulcimer manufacture is often conducted by small, family-run businesses located in the American South and particularly in Appalachia. John Bailey's book provides instructions for constructing a dulcimer:") '(#$ContentModelForPegFn #$L2R-SentenceLevelPeg-2) )
...
(define reload-cogbot-parser () (load "cynd/cogbot-parser.lisp") )

'(defvar *run-once-reload-cb* (make-process "Reload Cogbot Parser" #'reload-cogbot-parser))


(force-print ";; done loading cynd/cogbot-ke.lisp")
...
(find-or-create-constant "SubPegOfPegFn")
(cbnl-assert '(#$and
