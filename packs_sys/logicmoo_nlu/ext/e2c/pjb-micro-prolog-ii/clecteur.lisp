...
(defun alphanum (ch)                    ; symbol normal constituant
  (or (alphanumericp ch) (specialp ch)))
...
(defun status (nom n)                   ; n is the term's depth
  (if (= n 1)                           ; local if not yet global
      (unless (member nom *lvarglob*)
        (pushnew nom *lvarloc*))
      (progn (when (member nom *lvarloc*) ; else global
...
(defun read_simple (ch n)               ; next simple term
  (cond
    ((or (upper-case-p ch) (specialp ch)) (read_var ch n))
...
(defun c (l)                            ; codes once read
  (if (atom l)
      (if (member l *lvarloc*)
          (cons 'local (position l *lvarloc*))
          (if (member l *lvarglob*)
              (cons 'global (position l *lvarglob*))
              l))
