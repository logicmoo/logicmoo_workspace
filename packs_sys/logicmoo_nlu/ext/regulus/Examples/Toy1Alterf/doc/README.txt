...
Toy1Specialised is a version of the Toy1 Regulus grammar which is
built on top of the general Regulus grammar in $REGULUS/Grammars. The
associated recognizer is derived using grammar specialization. Like
Toy1, Toy1Specialized recognises simple commands and queries in a home
automation devices, e.g. "turn on the fan" or "is the light switched
on".

It is possible to run the grammar inside a toy spoken language
dialogue application or a toy English -> French speech translation
application. The rules and code are almost identical to that in the
Toy1 directory, and are reproduced here mainly for convenience.

The spoken dialogue application responds to the commands and
questions, and the speech translation application translates them into
French.
...
- Regulus/toy1_lex.regulus contains the English lexicon entries. These are combined
  with the general grammar to create the full grammar.

- scripts/toy1_specialised.cfg is the config file for the English grammar.
...
- scripts/Makefile is a makefile with two main targets:
  - "default" builds the Nuance grammar and then compiles it into a recognition package.
    The resulting package will be created as Generated/toy1_specialised_recogniser.	
...
- scripts/toy1_specialised_dialogue.cfg is the config file for the dialogue application grammar.

- Prolog/toy1_app.pl contains the top-level code for the application.
...
- scripts/toy1_specialised_slt.cfg is the English config file for the speech translation app.
...
5. TESTING THE ENGLISH RECOGNISER
