...
#ifndef MEDSLTDOCWINDOW_HPP_
#define MEDSLTDOCWINDOW_HPP_
...
public:
	...
	MedSLTDocWindow(MedSLTDocWindowObserver& observer);
	~MedSLTDocWindow();
	
	GtkWidget* CreateMenu(GtkAccelGroup *accel);
	
	MedSLTDocWindowObserver& GetObserver();	
	...
	// Set the patient's response
	void SetPatientResponse(string& result);
...
private:
	...
	MedSLTDocWindow(const MedSLTDocWindow&);
	MedSLTDocWindow& operator = (const MedSLTDocWindow&);
	
private:		
	
	MedSLTDocWindowObserver&	m_observer;
		
	GtkWindow*	m_window;
	GtkWidget*	m_understood;
	GtkWidget*	m_response;
	GtkWidget*	m_help;
