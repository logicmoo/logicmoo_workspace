...
text2ccg(Text,Lines):-!,
   tmp_file('boxer/logicmoo',X),
   file_directory_name(X,Dir),
   create_directory(Dir),
   atom_concat(X,'.txt',Txt),atom_concat(X,'.ccg',Ccg),atom_concat(X,'.log',Log),
...
atom2ccg(A,AA):-atom(A),atom_chars(A,Chars),not((member(C,['\\','[','/','(']),member(C,Chars))),!,string_lower(A,S),atom_string(AA,S),!.
...
:-thread_local toplevel_drsdata/1.
...
text2drsWhatnot(Text):-  with_dmsg(skip_dmsg,text2ccg(Text,Out)),
   ...
   expand_drs_args_real(true,Goal,Result),
   must(rename_vars(Result,ResultO)),
   portray_clause(text(Text):-ResultO).

rename_vars(Result,ResultOut):-rename_vars1(Result,ResultM),rename_vars2(ResultM,ResultOut).

rename_vars1(Result,ResultOut):-
   rename_vars_r(replacer1,Result,Result,ResultO),
   rename_vars_r(replacer2(ResultO),ResultO,ResultO,ResultOut),!.
rename_vars2(Result,ResultOut):-
   rename_vars_r(replacer3,Result,Result,ResultO),
   rename_vars_r(replacer4,ResultO,ResultO,ResultOut),!.
...
list_to_atom(Names,SName):- list_to_set(Names,Uniq),atomic_list_concat(Uniq,SName).
...
var_name_shorten(SName,OName):-atom_subst_change(SName,'Attribute','-J-',AName),var_name_shorten(AName,OName).
var_name_shorten(SName,OName):-atom_subst_change(SName,'nam','-T-',AName),var_name_shorten(AName,OName).
var_name_shorten(SName,OName):-atom_subst_change(SName,'--','-',AName),var_name_shorten(AName,OName).
var_name_shorten(SName,OName):-atom_subst_change(SName,'\'','',AName),var_name_shorten(AName,OName).
var_name_shorten(SName,OName):-atom_subst_change(SName,'-?','-',AName),var_name_shorten(AName,OName).
var_name_shorten(SName,OName):-atom_subst_change(SName,'?','',AName),var_name_shorten(AName,OName).
var_name_shorten(AName,AName).

name_from_term(_Var,CALL,_Name):-not(compound(CALL)),!,fail.
name_from_term(Var,isa(Var,Col),Name):-to_atom(Col,Name).
name_from_term(Var,named(Var,Col),Name):-to_atom(Col,Name).
...
rename_vars_l( Replacer,Comp,ResultIn,ResultO):-Comp =.. [L|List],rename_vars_l(Replacer,[L|List],ResultIn,ResultO).
rename_vars_l(_Replacer,_,Result,Result).


ato_atom(A,AA):-bad_char_replacer(Bad,_),atom_concat(NewA,Bad,A),!,ato_atom(NewA,AA).
ato_atom(A,A).
...
varify(YouA,IntA,Var):- to_atom(YouA,You),to_atom(IntA,Int), atom_concat(You,Int,YouInt),!,
      string_upper(YouInt, YOUINT),
      atom_string(YOUINTA,YOUINT),       
      atom_chars(YOUINTA,YOUINTC),!,
      predsubst(YOUINTC,bad_char_replacer,YOUINTD),!,
      atomic_list_concat(YOUINTD, VarS),var_name_shorten(VarS,VarI),
      atom_concat('?',VarI,Var),!.

bad_char_replacer('\'','-').
bad_char_replacer('_','-').
bad_char_replacer(',','').
bad_char_replacer('.','').
bad_char_replacer(A,''):-atom(A),not(letter(A)).
...
text2ColFn_pred(Text,Atom):-to_atom(Text,Atom),!.
text2ColFn_pred(Text,'Text2ColFn'(Text)).
...
assert_drs_holds(P,V,'NthOf'(PropList)):- forall(nth1(N,PropList,R),(atom_concat(P,N,PN),assert_drs_holds(PN,V,R))).
...
assert_drs_holds(tagV1,_, 0).
assert_drs_holds(tagV2,_, 0).
assert_drs_holds(tagV3,_, 0).
assert_drs_holds(tagV4,_, 0).
assert_drs_holds(namex,_, 'O').
assert_drs_holds(P,V,N):-assert_drsdata(true(holds(P,V,N))).
% ------------------------------------------------------------------
% assert_drs/N
% ------------------------------------------------------------------
assert_drs(A):-dmsg(assert_drs(A)), fail.
...
%   assert_drs(tagV(Marker,'NthOf'([Var|Args]))),
%    assert_drs(tagV(Marker,Var)),
   assert_drs(Sent),!.
...
assert_drs(isaVarPos(Var,Male,Pos)):-
  once(text2ColFn_pred(Male,MaleO)),
  assert_drs_holds(isa(Var,MaleO)), 
  once(pos2ColFn_pred(Pos,POSCol)),
  assert_drs_holds(isa(Var,POSCol)).
...
assert_drs(lam(V,E)):-isVarOrVAR(V),
 not(isVarOrVAR(E)),assert_drs(E).
...
:-get_current_drs(DRS),asserta(toplevel_drsdata(DRS)).
...
retractall_drs(DRS):-
  ignore((not(toplevel_drsdata(DRS)),
...
make_numbervar(Letter,Number,'$VAR'(N)):- string_concat(Letter,Number,Want),between(40,173000,N),with_output_to(string(Get),write('$VAR'(N))),Get=Want,!.
