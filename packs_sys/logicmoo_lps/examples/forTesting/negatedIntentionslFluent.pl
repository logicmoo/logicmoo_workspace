...
winningProposal(Candidate,NV) at T if 
	voteCount(Candidate,NV) at T, 
	not (voteCount(Other,ONV) at T, Other\==Candidate, ONV>NV).

report(Candidate,N,true) at T if voteCount(Candidate,N), winningProposal(Candidate,_).
report(Candidate,N,false) at T if voteCount(Candidate,N), not winningProposal(Candidate,_).

events report(_,_,_).
actions reported.

if report(clinton,4,true) at 2, report(trump,0,false) at 2 then reported.
if true then report(clinton,4,true) at 9, report(trump,0,false) at 9, reported.