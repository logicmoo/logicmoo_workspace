...
define_into_module_now(S,M,F//A):- number(A), !, A2 is A+2,define_into_module_now(S,M,F/A2).
define_into_module_now(_,M,F/A):- lmconfig:never_reexport_named(M,F,A),!.
define_into_module_now(_,M,F/A):- lmconfig:never_export_named(M,F,A),!.
