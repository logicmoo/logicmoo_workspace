...
load(Model) :- 
   ...
   for_each(thread(Thread,Tree), /* for all threads */
     from(threads([Model,Fiber|_])), /* from model fibers */
   ...
   for_each([Weight,Thread1,Thread2|X],
     from(weights([Model,Fiber|_])), /* load weights */
     do(insert([Model,Fiber|_],
 ...
 if(interrupt([Stream,Interrupt]), /* if interrupt */
    then((remove([Model,Stream|_],[clock|_]), /*    then clear stream */
         ...
         do((set([Model,Stream,sense|X],clock(1)), /* fire sensor */
          ...
          set([Model,Stream|_],seq(1)), /* reset stream */
          remove([Model,Stream|_],_>>>_>>>_)))). /* clear synchro */
...
reflect(Model):-for_each((Stream,I>>>Thread>>>Stimulus), /* for each stimulus */
 such_that(ist([Model,Stream|_],I>>>Thread>>>Stimulus)), /* retrieve synchro */
 do([Model,Stream|_]::::(I>>>Thread>>>Stimulus))). /* report synchro */

/* virtual machine stream reports */
[Model,Stream|_]::::(I>>>Thread>>>Stimulus) :-
 write(I>>>Thread>>>Stimulus),nl,
 remove([Model,Stream|_],I>>>Thread>>>Stimulus).
...
[Model,Stream,P|X]::::(_T>>>resume([P|X])) :- /* reenter thread */
...
[Model,Stream,P|X]::::(T>>>send([Q|Y])) :- /* send to [Q|Y] */
 ...
 if_not(ist([Model,Stream|_],weight([P|X],[Q|Y]--W)), /* if no weight */
        then(if(ist([Model,Stream|_],initial([P|X],[Q|Y]--W)), /*    then if weight */
            then(set([Model,Stream|_],weight([P|X],[Q|Y]--W))), /*    then attach */
            else(set([Model,Stream|_],weight([P|X],[Q|Y]--0)))))), /*    else inhibit */
...
[Model,Stream,P|X]::::(T>>>receive([Q|Y])) :- /* receive from [Q|Y]*/
 ...
 if(ist([Model,Stream,Q|Y],signal(send([P|X]))), /* if signal posted */
    then(if((ist([Model,Stream|_],weight([Q|Y],[P|X]--K)),K>0), /*    then if threshold */
...
[Model,Stream|Thread]::::(T>>>increment(weight([P,[F,X]],[Q,Y]))) :- /* increment weight */
 T1 is T+1,
 ist([Model,Stream|_],seq(I)), /* get global time */

 if_not(ist([Model,Stream|_],weight([P,[F,X]],[Q,Y]--W)), /* if no weight */
     then(if(ist([Model,Stream|_],initial([P,[F,X]],[Q,Y]--W)), /*    then if initial */
          then(set([Model,Stream|_],weight([P,[F,X]],[Q,Y]--W))), /*    then attach */
          else(set([Model,Stream|_],weight([P,[F,X]],[Q,Y]--0)))))), /*    else inhibit */

 if((ist([Model,Stream|_],weight([P,[F,X]],[Q,Y]--W)),W<1), /* if no threshold */
    then((W1 is W+1, /*    then increment */
       insert([Model,Stream|_],I>>>Thread>>>weight([P,[F,_]],[Q,Y]--W1)), /* report */
       set([Model,Stream|_],weight([P,[F,_]],[Q,Y]--W1))))), /* attach */
...
[Model,Stream|Thread]::::(T>>>decrement(weight([P,[F,X]],[Q,Y]))) :- /* decrement weight */
 ...
 ist([Model,Stream|_],seq(I)), /* get global time */
 if_not(ist([Model,Stream|_],weight([P,[F,X]],[Q,Y]--W)), /* if no weight */
      then(if(ist([Model,Stream|_],initial([P,[F,X]],[Q,Y]--W)), /*    then if weight */
         then(set([Model,Stream|_],weight([P,[F,X]],[Q,Y]--W))), /*    then attach */
         else(set([Model,Stream|_],weight([P,[F,X]],[Q,Y]--0)))))), /*    else inhibit */
 if((ist([Model,Stream|_],weight([P,[F,X]],[Q,Y]--W)),W>0), /* if no threshold */
...
[Model,Stream|Thread]::::(T>>>step(Y)) :- /* step */
 ...
 I1 is I+1,
 set([Model,Stream|_],seq(I1)),
 set([Model,Stream|Thread],clock(T1)). /* set clock */

[Model,Stream|Thread]::::(T>>>on(X)) :- /* detect */
 T1 is T+1,
 if(ist([Model|_],at(X)), /* if position */
    then(set([Model,Stream|_],on(X))), /*    then found */
      else(remove([Model,Stream|_],on(_)))), /*    else not found */
 set([Model,Stream|Thread],clock(T1)). /* set clock */
...
[Model,Stream|Thread]::::(T>>>choice(X)) :- /* random selection */
 ...
 insert([Model,Stream|_],I>>>Thread>>>fetch(R)),
 set([Model,Stream|Thread],clock(T1)). /* set clock */
...
[Model,Stream|Thread]::::(T>>>scan(detect([F,X]),move(Y))) :- /* scan synchro */
 ...
 if((ist(Model,[Stream,I1]>>>detect([F,X])>>>excite(on([F,X]))), /* if synchro */
    ...
    then((set([Model,Stream|Thread],sync(detect([F,X]),move(Y))), /* set synchro */
          insert([Model,Stream|_],I>>>Thread>>>sync(detect([F,X]),move(Y)))))), /* report synchro */
 set([Model,Stream|Thread],clock(T1)). /* set clock */
...
[Model,Stream|Thread]::::(T>>>effector(P)) :- /* virtual effector */
 T1 is T+1,
 ist([Model,Stream|_],seq(I)), /* get global time */
 insert([Model,Stream|_],I>>>Thread>>>effector(P)),
 set([Model,Stream|Thread],clock(T1)). /* set clock */
