...
:-consult(compiler_cleaner).
 ...
 sense(Model) :- if(interrupt(Stream(Interrupt)), /* if interrupt      */
      then((remove(Model(Stream)(_),clock(|_)), /* then clear stream */
            remove(Model(Stream)(_),excite(|_)), remove(Model(Stream)(_),inhibit(|_)), remove(Model(Stream)(_),signal(|_)), 
...
(T:Instruction))).
 ...
 reflect(Model):-for_each((Stream,I:Thread:Stimulus), /* for each stimulus */
      ...
      do(Model(Stream). 
(I:Thread:Stimulus))). /* report synchro    */
...
Model(Stream).
(I:Thread:Stimulus) :-    write(I:Thread:Stimulus),nl, remove(Model(Stream),I:Thread:Stimulus).
...
Model(Stream)(P(|X)).
(T:resume(P(|X))) :-                     /* reenter thread    */
               set(Model(Stream)(P(|X)),clock(1)).
             /* reset P(|X) clock */
 
 
Model(Stream)(P(|X)).
(T:end) :-                    /* end active thread */
               remove(Model(Stream)(P(|X)),clock(T)).
                     /* deactive thread   */
 
 
Model(Stream)(P(|X)).
(T:send(Q(|Y))) :-                       /* send to Q(|Y)     */
               T1 is T+1, 
...
Model(Stream)(P(|X)).
(T:receive(Q(|Y))) :-                    /* receive from Q(|Y)*/
               T1 is T+1, 
...
Model(Stream)(Thread).
(T:increment(weight(P(F(X)),Q(Y)))) :-             /* increment weight  */
               T1 is T+1, 
...
Model(Stream)(Thread).
(T:decrement(weight(P(F(X)),Q(Y)))) :-             /* decrement weight  */
               T1 is T+1, 
              ist(Model(Stream),seq(I)), /* get global time   */
...
(T:step(Y)) :-                          /* step              */
              ...
              if(Y=forward, /* get direction     */
               then(F=right), else(F=left)), 
 
              if(ist(Model(_),at(F(X))), /* get position      */
               then(X1 is X+1), else(X1 is 1)), 
...
Model(Stream)(Thread).(T:check(P(|X))) :-                     /* check if P(|X)    */
               T1 is T+1, 
              ist(Model(Stream),seq(I)), /* get global time   */
               remove(Model(Stream)(Thread),excite(P(|X))), remove(Model(Stream)(Thread),inhibit(P(|X))), 
              if(ist(Model(Stream),P(|X)), /* if P(|X)          */
               ...
               else((set(Model(Stream)(Thread),inhibit(P(|X))), insert(Model(Stream),I:Thread:inhibit(P(|X))), 
...
Model(Stream)(Thread).
(T:scan(detect(F(X)),move(Y))) :-                  /* scan synchro      */
               T1 is T+1, 
              ist(Model(Stream),seq(I)), /* get global time   */
...
insert(C,P) :- assert(instance(C,P)).
 ...
 for_each(X, such_that(F), 
do(P)) :- findall(X,F,L), forall(member(X,L),P).
 
for_each(X, from(F), 
do(P)) :- forall(F:L, forall(member(X,L),P)).
...
[thread(sense(F(X)), [merge(ltd(sense(F(X)), learn(F))), merge(ltp(sense(F(X)), recall(Y))), 
    
   send(learn(F)), fire(detect(F(X))), send(recall(Y))]), 
 ...
 thread(detect(F(X)), 
  ...
  thread(recall(Y), [receive(sense(F(X))), fire(ltd(sense(F(X)),learn(F))), fire(move(Y))]), 
  thread(move(Y), [step(Y), check(at(F(_))), 
   ((excite(at(F(_)))  | [resume(move(Y))]);     (inhibit(at(F(_))) | [effector(stop(Y))]))])].
 
 threads(robot): [thread(ltp(Q,R), [join(Q), 
   increment(weight(Q,R))]), 
 
 thread(ltd(Q,R), [join(Q), decrement(weight(Q,R))])].
 
 weights(robot(cleaner(A,B))): [initial(sense(_),learn(_))(1), initial(recall(_),do(_))(1)].
 /* example run  
 consult(cleaner).
 load(robot).
  run(robot).
...
9 : detect(left(2)) : inhibit(on(left(2))) 9 : detect(left(2)) : inhibit(on(left(2))) cleaner(forward,backward)([sensor(left(1))]).
...
/* BNF syntax 
<tree> ::= [] || <sequence> || [<alternative>] <sequence> ::= [<instruction>|<tree>] <alternative> ::= <branch> || (<branch>;<alternative>) <branch> ::= (<guard>|<tree>)  
*/
...
compileSequence([Instruction|Tree],Guard,T,Code) :- T1 is T+1, 
...
compileBranch((Guard1|Tree),Guard2,T,Code) :- 
  (Guard2=true -> Guard=Guard1; Guard=(Guard1,Guard2)), compileTree(Tree,Guard,T,Code).
...
(T:scan(detect(F(X)),move(Y))) :- /* scan synchro */
 ...
 ist(Model(Stream),seq(I)), /* get global time */
  remove(Model(Stream)(Thread),sync(detect(F(X)),move(Y))), /* remove synchro */
  if((ist(Model,Stream(I1):detect(F(X)):excite(on(F(X)))), /* if synchro */
  ist(Model,Stream(I2):move(Y):excite(at(F(X)))), 
I1=I2), 
 then((set(Model(Stream)(Thread),sync(detect(F(X)),move(Y))), /* set synchro */
  insert(Model(Stream),I:Thread:sync(detect(F(X)),move(Y)))))), /* report synchro */
