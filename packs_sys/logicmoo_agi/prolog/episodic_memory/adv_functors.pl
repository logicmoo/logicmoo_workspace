...
type_functor(event, percept(agent, exit_list(domain, fn, in, dest, list(exit)))). % paths noticable
type_functor(event, percept(agent, child_list(domain, dest, domrel, depth, list(inst)))).
