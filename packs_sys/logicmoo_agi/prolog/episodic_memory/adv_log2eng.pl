...
logic2eng(Context, dO('go_dir', Agent, How, Dir), [ How, Dir]):- Context=Agent.
logic2eng(_Context, dO('go_dir', Agent, How, Dir), [ Agent, How, Dir]).
...
logic2eng(Context, percept_props(Agent, see, Object, _Depth, PropList), [cap(subj(Agent)), sees | English ] ) :-
 logic2eng(Context, dO('props', Object, PropList), English).
...
logic2eng(_Agent, dO('props', _Object, []), '<..>' ) :- !.
logic2eng(_Agent, dO('props', Object, PropList), English ) :- list2eng(['.'='.'], Object, PropList, English).
...
logic2eng(Context, did_emote(Speaker, EmoteType, Audience, Eng), [ 'happened: '| Rest]) :- !,
 logic2eng(Context, dO('emote', Speaker, EmoteType, Audience, Eng), Rest).

logic2eng(_, dO('emote', Speaker, act, '*'(Place), Eng), [the(Speaker), at, Place, Text]) :- !,
 eng2txt(Speaker, Speaker, Eng, Text).
logic2eng(_, dO('emote', Speaker, act, Audience, Eng), [Audience, notices, the(Speaker), Text]) :-
 eng2txt(Speaker, Speaker, Eng, Text).
logic2eng(_, dO('emote', Speaker, EmoteType, Audience, Eng), [cap(subj(Speaker)), es(EmoteType), 'to', Audience, ', "', Text, '"']) :-
...
logic2eng(_Aobj, cant( dO('move', _Agent, It)), [ It, aux( be), 'immobile']).
logic2eng(_Aobj, cant( dO('take', _Agent, It)), [ It, aux( be), 'untakeable']).
...
% logic2eng( Obj, Prop, English):- Prop =..[N, Obj1, A| VRange], Obj1==Obj, Prop2 =..[N, A| VRange], logic2eng( Obj, Prop2, English).
logic2eng( Obj, Prop, English):- Prop =..[N, V, T| VRange], T==t, Prop2 =..[N, V| VRange], logic2eng( Obj, Prop2, English).
logic2eng(_Obj, sub__do_examine(Who, Sense, Prep, Where, 3), [cap(subj(Who)), es(Sense), Prep, Where]):-!.
...
%logic2eng(_Obj, Prop, [String]):- compound(Prop), !, String=''. % format(atom(String), ' \n {{ ~q. }}\n ', [Prop]), !.
%
% sub__do_examine(_Player_1, see, in, living_room, 3)
