...
% try to run the intend(Agent, act3('auto',Agent,[])) command
maybe_autonomous_decide_goal_action(Agent, Mem0, Mem1) :-
 add_intent( Agent, intend(Agent, act3('auto',Agent,[])), Mem0, Mem1).
...
autonomous_decide_unexplored_exit(Agent, Mem0, Mem2) :-
 ...
 add_intent(Agent, intend(Agent, act3('go__dir',Agent,[ walk, Dir])), Mem0, Mem1),
 add_intent(Agent, intend(Agent, act3('go__dir',Agent,[ walk, Prev])), Mem1, Mem2).
autonomous_decide_unexplored_exit(Agent, Mem0, Mem1) :-
 agent_thought_model(Agent, ModelData, Mem0),
 in_agent_model(Agent, h(in, Agent, Here), ModelData),
...
autonomous_decide_unexplored_object(Agent, Mem0, Mem2) :-
 ...
 add_intent( Agent, intend(Agent, act3('open',Agent,[ Object])), Mem0, Mem1),
 add_intent( Agent, intend(Agent, act3('examine',Agent,[ see, Object])), Mem1, Mem2).

autonomous_decide_unexplored_object(Agent, Mem0, Mem1) :-  fail,
 agent_thought_model(Agent, ModelData, Mem0),
 in_agent_model(Agent, h(A, '<mystery>'(W, B, C), Object), ModelData),
 add_intent( Agent, intend(Agent, act3('make_true',Agent,[ ~(h(A, '<mystery>'(W, B, C), Object))])), Mem0, Mem1).
...
autonomous_decide_silly_emoter_action(Agent, Mem0, Mem1) :-
 ...
 add_intent( Agent, intend(Agent, act3('emote',Agent,[ act, *, Msg])), Mem0, Mem1).


always_action( intend(_, act3('go__dir',_,[ _, _]))).
...
consider_request(Requester, Agent, Query, M0, M1) :-
 ...
 add_intent( Agent, intend(Agent, act3('emote',Agent,[ say, Requester, Answer])), M0, M1).
...
consider_request(_Speaker, Agent, intend(Agent, act3('put',Agent,[ Thing, Relation, Where])), M0, M) :-
 add_goal(Agent, h(Relation, Thing, Where), M0, M).
consider_request(_Speaker, Agent, intend(Agent, act3('take',Agent,[ Thing])), M0, M) :-
 add_goal(Agent, h(held_by, Thing, Agent), M0, M).
consider_request(_Speaker, Agent, intend(Agent, act3('drop',Agent,[ Object])), M0, M1) :-
