...
menu_cmd1(_,'s','              Try to (s)olve based on training',(solve_via_scene_change)).
menu_cmd1(_,'S','                  or (S)olve confirming it works on training pairs',(solve_via_scene_change)).
...
maybe_pair_is_simple_repaint(I,O):-
  ...
  mapgrid(fg_to_else(fg,unkC),II,III),
  mapgrid(fg_to_else(fg,unkC),OO,OOO),!,
  III=@=OOO.
...
system:load_last_test_name:-  
  muarc:arc_settings_filename_for_load(Filename),
...
system:save_last_test_name:- /*notrace*/(catch(save_last_test_name_now,E,wdmsg(E))),!.
...
system:save_last_test_name(TestID):- /*notrace*/(catch(save_last_test_name_now(TestID),E,wdmsg(E))),!.
system:save_last_test_name_now(TestID):- 
 must_det_ll((
 (muarc:arc_settings_filename_for_load(FilenameL)-> true ; muarc:arc_settings_filename(FilenameL)),
  save_at(TestID,FilenameL),
  once((muarc:arc_settings_filename(Filename),FilenameL\==Filename)),
  save_at(TestID,Filename))),!.
  
save_at(TestID,Filename):- /*notrace*/(ignore(( is_valid_testname(TestID),
    (exists_file(Filename)->true;wdmsg(saving_settings_at(Filename))),
     tell(Filename),format('~n~q.~n',[TestID]),told))).



muarc:arc_settings_filename_for_load(Filename):- muarc:arc_settings_filename(Filename), exists_file(Filename).
muarc:arc_settings_filename(Filename):- suggest_setting_filename(File), 
 (exists_file(File)->absolute_file_name(File,Filename,[access(append),file_errors(fail),expand(true)]);
   absolute_file_name(File,Filename)).
  
suggest_setting_filename(A):- suggest_rel_setting_filename(Z),(add_ptty(Z,A);Z=A).
add_ptty(Z,A):- getenv('PTTY',PTTY),atomic_list_concat(List,'/',PTTY),atomic_list_concat([Z|List],'_',A).
suggest_rel_setting_filename('../shared/muarc_tmp/current_test').
suggest_rel_setting_filename('~/.arc_current_test').
suggest_rel_setting_filename('/tmp/.arc_current_test').
