...
is_bg_object_really(Obj):- \+ sub_var(cc(unkC,0),Obj), sub_var(cc(fg,0),Obj).
...
is_fg_object(Obj):- sub_var(cc(unkC,0),Obj),!.
...
physical_points(GPoints,OPoints):- 
   my_partition(sub_var(unkC),GPoints,BGPoints,OPoints),
   BGPoints\==[],OPoints\==[],!.
...
colors_cc(G,[cc(Black,0),cc(fg,0),cc(unkC,0)]):-  G==[],!,get_black(Black).
...
add_summary_colors(CC,CCO):- total_summary_colors(0,0,0,BG,FG,Vars,CC),
   include(is_cc_n('=<'(0)),[cc(unkC,BG),cc(fg,FG),cc(plain_var,Vars)],CCE),append(CC,CCE,CCO).
...
total_summary_colors_e(_BG,FG,Vars,-1,FG,Vars,cc(unkC,_)).
...
ngrid_syms(NGrid,NSymCounts):- fix_tt_juctions(NGrid,TGrid90),
  subst_syms(unkC,TGrid90,GFlatSyms),get_ccs(GFlatSyms,Syms),
...
create_sym_vectors(Syms,Counts):-
  findall(sym_counts(Sym,Count),
    (vector_pairs(T,ListU),sort(ListU,List),
     atomic_list_concat([sym,T|List],'_',Sym),
     total_ccs(10,List,Syms,Count)),Counts).

total_ccs(_,[],_,0).
total_ccs(N,[S|List],Syms,Count):- 
 N2 is N * 10, 
 total_ccs(N2,List,Syms,CT),!,
 (((member(cc(S,C),Syms),nonvar(C))->true;
   findall_count(_,member(S,Syms),C))-> (Count is (CT + (C*N))); Count=CT).
...
/*
find_syms(corners,N2):- name(N2,[355]). %0
find_syms(corners,N2):- name(N2,[356]). 
find_syms(corners,N2):- name(N2,[357]). 
find_syms(corners,N2):- name(N2,[358]). 
find_syms(corners,N2):- name(N2,[359]). 
find_syms(corners,N2):- name(N2,[360]). %5
*/
find_syms(corners,N2):- neih_syms(_,N2).
...
cell_syms(_IBGC,Sym,Sym):- number(Sym),!.
...
black_vs_bg(C,O):- plain_var(C),!,O = unkC.
black_vs_bg(C,O):- C == unkC,!, O = _.
black_vs_bg(C,C).
...
%guess_shape(GridH,GridV,GridIn,LocalGrid,I,Empty,N,H,V,[cc(Black,_)|Rest],Points, fbc(FGB)):- Rest == [], ((is_fg_color(Black);Black==fg)->FGB=fg;FGB=unkC).
