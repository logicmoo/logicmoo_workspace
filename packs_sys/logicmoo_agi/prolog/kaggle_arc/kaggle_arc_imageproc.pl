...
remove_global_cpoints(Point,Grid,GridO):- as_hv_point(H,V,Old,Point), nonvar(Old), get_bgc(C),!,replace_local_point(H,V,C,Old,Grid,GridO).
...
%replace_each_local(_H,_V,NewC,OldC,Point,G,GO):-nonvar(G),!,G=obj(L),is_list(L),
