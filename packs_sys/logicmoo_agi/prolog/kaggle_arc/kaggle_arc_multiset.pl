...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% top level predicate: turns a a_grouping into its multiset, if defined
% steps are
...
%%% for a_grouping G, MCCs is a maximal sorted list of correlated attributes on G
...
%%%%%%%%% attributes correlated on a_grouping
...
% attributes are correlated on a_grouping G if they appear in the same given
...
% explicit grouping size
given_size_index_integer(Group,AttList,Size,Index) :-
	size(Group,Size),
	top_index(AttList,Index).
% implicit a_grouping size 1
