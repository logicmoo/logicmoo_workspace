...
:- user:import( pce_principal:get/3).
...
individuation_macros(do_ending, [
 ...
 %keep_only_shown(1),
 %remove_if_prop(and(cc(unkC,1))),
 remove_illegals,
 %combine_if_prop(and(cc(unkC,1),)),
...
i_to_o_is_none_some_some(VM):-    is_output_vm(VM),!,
  ...
  individuate2(_,[keypads],VM.id,Intersect,AddObjs),
...
i_to_o_is_none_some_some_in(VM):-
 ...
 if_t(LenMass>0,
  ...
  gset(VM.grid)=Zero,
  % individuate_3(i_colormass,Zero,Library),
...
mono_i_to_o_is_none_some_none_in(VM):-
 ...
 if_t(LenMass>0,
  ...
  o_minus_i(Zero), \+ no_fg_mass(Zero),
  grid_to_points(Zero,Points),
  gset(VM.grid)=Zero,% individuate_3(i_colormass,Zero,Library),
  gset(VM.lo_points)=Points,
  gset(VM.objs)=[],
  one_fti(VM,colormass),
  Library1 = VM.objs,
...
guess_how(HOW,I1,O1,Stuff1,Stuff2):-
  ...
  must_det_ll((
    ...
    once((individuate_3(HOW,I,Stuff1))),
    once((individuate_3(HOW,O,Stuff2))))),
...
individuation_macros(i_complete_generic1, [i_complete_generic1a,do_ending]).

individuation_macros(i_complete_generic1a, [
...
progress(P):- nop(pp_i(progress(P))).
progress(C,P):- nop(pp_i(C,progress(P))).
...
show_individuated_nonpair(PairName,ROptions,GridIn,Grid,InC):-
 must_det_ll((
  ...
  length(InC,Len),
  pp_i(show_individuated_nonpair=Len),
  if_t(GridIn\==Grid,
...
is_fti_step(must_indv_omem_points).
% =====================================================================
...
skip_some(VM):- 
  ...
  print_ss(skip_some3,VM.objs,IndivPointSet),
...
colormass_3(VM):- 
  ...
  append(FirstTwo,GPoints,IndivPoints),
  sort(IndivPoints,IndivPointSet),
  nop(print_ss(cm3,VM.objs,IndivPointSet)),
  length(IndivPointSet,Len),Len>=3,
  \+ (member(C-P3,IndivPointSet),member(C-P4,LeftOver),is_adjacent_point(P3,Card,P4),n_s_e_w(Card)),
...
obj_call(subst_color(fg,Color),O1,O2):- !, map_pred1(replace_term(is_ifg_color,=(Color)),O1,O2).
...
like_object(Ans,Out,ObjO):- 
  ...
  must_grid_to_gid(Out,GID),grid_size(Out,GH,GV), 
...
%individuate_second_pass(Grid,IndvS):- individuate_3([second_pass],Grid,IndvS).
...
get_individuated_cache(_TID,ROptions,GOID,IndvS):- nonvar(ROptions),
  ground(GOID), \+ luser_getval(use_individuated_cache,false), saved_group(individuate(GOID,ROptions),IndvS),!.

individuate_1(VM):- individuate1(VM,VM.start_options,VM.grid,InC),set(VM.objs)=InC,!.
individuate_c(VM):- individuate1(VM,complete,VM.grid,InC),set(VM.objs)=InC,!.
%individuate_1(VM):-  individuate_2(VM.start_options, VM),!.

individuate_2(ROptions,VM):-  individuate_3(ROptions,VM,_),!.
...
individuate1(VM,ROptions,GridIn,IndvS):- 
  ...
  once((delistify_single_element(ROptions,NamedOpts),
        maybe_save_grouped(individuate(GOID,NamedOpts),IndvS))).

must_grid_to_gid(GridIn,GOID):-
  % print_grid(must_grid_to_gid(GOID),GridIn),
   must_det_ll(grid_to_gid(GridIn,GOID)).
...
mono_cell_minus_cell1(BG,I,O,M):- is_ifg_color(I),is_ifg_color(O), M=BG.
...
individuate_nonpair(ROptions,In0,IndvSI):- 
  duplicate_term(In0,In),
  into_grid(In,InG0), 
  compile_and_save_current_test_hints([for(InG0)]),!,
  duplicate_term(InG0,InG),
  individuate_3(ROptions,InG,IndvSI).
...
individuate_two_grids(ROptions,GridIn,GridOut,IndvSI,IndvSO):- 
  ...
  must_grid_to_gid(GridIn,OIDIn), 
  must_grid_to_gid(GridOut,OIDOut),
...
individuate_two_grids_now_X(OID_In_Out,ROptions,Grid_In,Grid_Out,VM_In,VM_Out,Grid_In,Grid_Out,Objs_In,Objs_Out):- 
 must_det_ll((   
  ...
  maybe_save_grouped(individuate(OID_In_Out,ROptions),IndvS))),!.
/*
...
individuate_two_grids_now_X(OID_In_Out,ROptions,Grid_In,Grid_Out,VM_In,VM_Out,Grid_InX,Grid_OutX,Objs_InX,Objs_OutX):- 
 must_det_ll((
  ...
  worker_output((
   with_other_grid(Grid_OutX,do_individuate(ID,VM_In,ROptions,Grid_InX,Objs_In)),!, 

     gset(VM_Out.robjs) = Objs_In,
   with_other_grid(Grid_InX,do_individuate(ID,VM_Out,ROptions,Grid_OutX,Objs_Out)),!,
...
individuate2(_VM,ROptions,GOID,_GridIn,IndvS):- nonvar(GOID), 
  ...
  !.
individuate2(VM,ROptions,GOID,GridIn,IndvS):-
  do_individuate(GOID,VM,ROptions,GridIn,LF),!,
...
do_individuate(ID,VM, ROptions, GridIn,LFO):- must_be_free(LF), 
...
recolor_bg_or_fg(I,O):- is_ifg_color(I),!,O=fg.
recolor_bg_or_fg(I,O):- is_bg_color(I),!,O=unkC.
...
%is_ifg_color(FG):- FG==black.
is_ifg_color(FG):- is_fg_color(FG).
...
mono_shaped(_FG,_BG,Cell,unkC):- plain_var(Cell),!.
mono_shaped(_FG,_BG,Cell,unkC):- Cell==unkC,!.
mono_shaped(_FG, BG,Cell,NewCell):- is_bg_color(Cell),!,nop(decl_many_bg_colors(NewCell)),NewCell=BG.
mono_shaped( FG,_BG,Cell,NewCell):- is_ifg_color(Cell),!,nop(decl_many_fg_colors(_NewCell)),NewCell=FG.
...
fg_shaped(_BGCs,Cell,fg):- is_ifg_color(Cell),!.
fg_shaped(_BGCs,Cell,Cell):- attvar(Cell),!.
fg_shaped(_BGCs,Cell,unkC):- plain_var(Cell),!.
fg_shaped(_BGCs,_Cell,black).
%fg_shaped(Cell,NewCell):- is_ifg_color(Cell),!,decl_many_fg_colors(NewCell),NewCell=Cell.

fg_shapes(Shape,VM):-
  Grid = VM.grid,
  colors_across(Grid,Silvers),
  get_black(Cell),
  BGCs = [Cell,wbg,unkC|Silvers],
...
fgc_as_color(C,BG,W):- is_ifg_color(BG),!,cvt_to_color(C,W).
fgc_as_color(_,W,W).
...
sub_self(NewGrid,NewOther,TODO,VM,FoundObjs):-  
 print_grid(pp_i(doing(TODO)),NewGrid),
 must_det_ll((
...
% print_grid(sub_self(TODO),NewGrid),
 with_other_grid(NewOther,
   (/*itrace,*/do_individuate(ID,_,TODO,NewGrid,FoundObjs))),
...
fg_intersectiond_mono1(_,This,This,That):- is_ifg_color(This),is_ifg_color(That).
...
bigger_grid_contains_other_i_o_1(out,VM):- 
 ...
 current_pair_io(I,O), O=@=Grid,
  arc_common_property(containsAll(o-i)),
  individuate_3(generic_nsew_colormass,I,IObjs),
...
bigger_grid_contains_other_i_o_2(in,VM):- 
 ...
 other_grid(Grid,Other), is_grid(Other),
  ...
  individuate_3(generic_nsew_colormass,O,OObjs),
...
obj_gpoints(_Grid,OBJ,OH,OV,H,V,GOPoints):- is_grid(OBJ),!,
 ...
 ((member(W-HV1,OPoints),is_ifg_color(W)) -> OPoints=OOPoints ; OOPoints=[black-HV1|OPoints]),
 ((member(W-HV2,OOPoints),is_ifg_color(W)) -> OOPoints=OOOOPoints ; OOOOPoints=[black-HV2|OOPoints]),
...
%fg_abtractiond(Cell,NewCell):- is_ifg_color(Cell),!,decl_many_fg_colors(NewCell),NewCell=Cell.
...
fg_abtractions(Subtraction,VM):-
 ...
 must_det_ll((
  ...
  individuate2(_,Subtraction,NewGrid,VM.id,FoundObjs),
  set_vm(VMS),
  ReColored = FoundObjs,
 ...
 must_det_ll((
  ...
  if_t(Mass>0,
   ((individuate2(_,ROptions,GOID,GridIn,IndvS0),
    ...
    once((delistify_single_element(ROptions,NamedOpts), 
      (maybe_save_grouped(individuate(GOID,NamedOpts),IndvSL)))),!,
    TID = VM.id,
...
%individuateR(Grid,complete,Objs):- individuate_3(complete,Grid,Objs).
...
%  individuate_3(Name,Grid,Objs),Objs\==[].
...
one_fti(VM,glyphic):-
 must_det_ll((
  ...
  maybe_save_grouped(individuate(VM.gid,glyphic),IndvList))))))).
...
one_fti(VM,named_grid_props(Name)):-
  H=VM.h,V=VM.v,
  Grid= VM.start_grid,
  hv_point_value(1,1,Grid,PointNW),
  hv_point_value(1,V,Grid,PointSW),
  ...
  hv_point_value(H,V,Grid,PointSE),
  grid_props(Grid,Props),
  append(Props,[mass(0),vis2D(H,V),birth(named_grid_props(Name)),loc2D(1,1),iz(flag(always_keep)),iz(media(image)),iz(flag(hidden))],AllProps),
...
whole_into_obj(VM,Grid,Whole):- 
 must_det_ll((
  ...
  maybe_save_grouped(individuate(VM.gid,whole),[Whole]),learn_hybrid_shape(pair,Whole))).
  /*
  if_t(Len>=0,
...
ok_color_with(C1,C2):- my_assertion(is_color(C1)), 
  (plain_var(C2)->freeze(C2,ok_color_with(C1,C2));
    (luser_getval(color_mode,monochrome) -> 
     (is_ifg_color(C1)-> is_ifg_color(C2);is_bg_color(C2)) 
     ; (\+ (C1 \= C2)))).
*/
...
individuation_macros(i_pbox,[pbox_vm,generic_nsew_colormass,do_ending]).
...
%individuator(i_by_color,[by_color(0), by_color(0,wbg), by_color(0,fg),  reset_points, by_color(1,black),by_color(1,unkC), by_color(1,fg),/* ,*/[]]).
individuator(i_by_color,[by_color(0,wbg), by_color(1,black),by_color(1,unkC), by_color(1,fg)]).
...
individuator(i_by_color,[by_color(0), by_color(0,wbg), by_color(0,fg), 
  reset_points, by_color(1,black),by_color(1,unkC), by_color(1,fg),/* ,*/[]]).
...
%individuator(i_fgbg,[by_color(1,unkC), by_color(1,fg),do_ending]).
...
%:- expand_file_name('../MOMMY/*individuation.pl',Files),maplist(ensure_loaded,Files). 
