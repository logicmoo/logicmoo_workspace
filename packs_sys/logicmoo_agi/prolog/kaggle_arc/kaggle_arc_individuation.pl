...
individuator(i_common,[subshape_both(v,common_shape_lib),do_ending]).
...
bg_shapes(Shape,VM):-
 must_det_ll((
  ...
  %extra_info(np=NewPoints), extra_info(ng=NewGrid),
  run_fti(VM,Shape),
...
as_bgoid(Fg,Wbg,Gridoid,BGoid):- into_monochrome(consider_fg(Fg),consider_bg(Wbg),Gridoid,BGoid).
