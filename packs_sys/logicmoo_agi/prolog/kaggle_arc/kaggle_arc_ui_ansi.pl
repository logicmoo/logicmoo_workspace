...
into_ss_string(A+B,ss(Len,LAB)):- into_ss_string(A,ss(Len,LA)),into_ss_string(B,ss(_,LB)), append(LA,LB,LAB).
...
into_ss_string(G,SS):- into_grid(G,GR),is_grid(GR),!,wots(S,print_grid_ss(GR)),!,into_ss_string(S,SS).
