...
pbox_vm(VM):- !,
 ...
 must_det_ll((
  ...
  %points_to_grid(VM.h,VM.v,VM.lo_points,GridI0),
  %mass(GridI0,Mass), ((Mass>0) -> RealGrid=GridI0 ; 
  (real_grid(VM,RealGrid)),
    
  %GridI0=VM.grid,
  pbox_vm(GH,GV,RealGrid,Sizes_S_L,VM))).

real_grid(VM,RealGrid):- 
 testid_name_num_io(VM.id,TestID,Example,Num,IO), kaggle_arc_io(TestID,Example+Num,IO,RealGrid),!.
...
pbox_vm_special_sizes(Objs,A,GH,GV,RealGrid,Sizes_S_L,VM):- 
 must_det_ll((
  mass(RealGrid,Mass), ((Mass>0) -> RealGrid=GridI0 ; real_grid(VM,GridI0)),
...
pbox_vm(GH,GV,GridI000,Sizes_S_L,VM):-
   get_black(Black),
   subst(GridI000,wbg,Black,RealGrid),
   mass(RealGrid,Mass), ((Mass>0) -> RealGrid=GridI0 ; real_grid(VM,GridI0)),
   % \+ \+ (length(Sizes_S_L,A),list_upto(20,Sizes_S_L,Some),u_dmsg(use_sizes(A)),print(Some)),
  
   subst_2L([wbg,unkC],[Black,Black],GridI0,GridI),
  ...
  RecIn = [],
  begin_i_pbox_l(RecIn,_RecOut,Grid,NSEW,XSG,Points,  Points1,VM,s_l(p1),Sizes_S_L), if_t(Points\==Points1,gset(VM.lo_points)=Points1),
...
begin_i_pbox_l(RecIn,Recs,Grid,NSEW,XSG,Points5,Points9,VM,S_L,Sizes_S_L):-
  copy_term(NSEW+XSG,CNSEW+CXSG),
  length(Sizes_S_L,Len),
  debug_m(indiv(pbox),begin(S_L,Len)), my_time(must_det_ll((i_pbox_l(RecIn,Recs,Grid,CNSEW,CXSG,Points5,Points9,VM,S_L,Sizes_S_L)))),
...
at_least_two_colors(IBorderS,RColors):- flatten_set_bf(IBorderS,Colors),
  ...
  RColors = [_,_|_], \+ member(unkC,RColors).
...
i_pbox_l(SoFarI,SoFarOut,Grid,NSEW,XSG,Points,Points9,VM,L_S,OUT):- i_pbox_l_mc(SoFarI,SoFarOut,Grid,NSEW,XSG,Points,Points9,VM,L_S,OUT),!.
...
i_pbox_l_mc(SoFarI,SoFarOut,Grid,NSEW,XSG,Points,Points9,VM,L_S,OUT):- OUT = [Size2D|Sizes],
  Size2D = size2D(H,V),
  once(H<17;V<17),
  once(H>1;V>1),
...
/*  (Which=@=find -> (IsRim=filltype(solid),OBJ=Find,OH is FX-1, OV is FY-1) ;
  ...
  %once((
   OBJ \=@=Grid, OBJ \==[],  %CACHE.objFound = OBJ, %CACHE.which = Which, %CACHE.objMade = Obj, %append(SoFarI,[Rec],SoFarMid),
%  fix_obj(
  OBJ\=Grid,
  obj_gpoints(Grid,OBJ,OH,OV,GOPoints),
  my_partition(is_fg_point,GOPoints,FGGOPoints,_BGGOPoints),
  (FGGOPoints\==[] -> (intersection(Points,FGGOPoints,Intersection,LeftOver,_Unknown), Intersection\==[]) ;
    LeftOver = Points),
 ...
 must_det_ll((
  ...
  debug_c(indiv(pbox),
   ((\+ \+ ((
    ...
    %grid_size(OBJ,HH,VV), EH is OH+HH-1,EV is OV+VV-1, clip(OH,OV,EH,EV,Grid,OGrid), print_side_by_side(cyan,OGrid,SY,_,OBJ,F),
      ...
      writeg([Rec,o=loc2D(OH,OV),
              WHY,L_S,Size2D,CACHE, 
              centerS=CenterS,insideS=InsideS,findS=FindS,
  ...
  points_to_grid(VM.h,VM.v,LeftOver,NextGrid),
  debug_c(indiv(pbox),  print_grid(pbox_lefover,NextGrid)),
  
  i_pbox_l([Rec|SoFarI],SoFarOut,NextGrid,NSEW,XSG,LeftOver,Points9,VM,L_S,[Size2D|Sizes]))). 
...
found_box(Grid,L_S,NSEW,FX,FY,Find,Center,Inside,CACHE,XSG,H,V,CenterS,InsideS,FindS,IBorderS,OBordefrS,   inside, Why):- fail,
  (H>2,V>2),
  CACHE.h=TGX,CACHE.v=TGY,CACHE.ogx=OGX,CACHE.ogy=OGY,
...
found_box(Grid,L_S,NSEW,FX,FY,Find,Center,Inside,CACHE,XSG,H,V,CenterS,InsideS,FindS,IBorderS,OBorderS,   inside, 
  ...
  %(H>1,V>1), 
  NSEW=[A,B,L,D], A=B,L=D,A=L, flatten_set_bf(CACHE.oBorder,OBorderAll),OBorderAll=[C], 
