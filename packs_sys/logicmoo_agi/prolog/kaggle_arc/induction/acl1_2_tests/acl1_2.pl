...
Caracteristics:
	test of examples:
	first try Prolog derivation, if fails,
	try derivation Prolog derivation of the non abducible part and
	try the derivation of the conjunction of abd in every possible order1

	Negative examples tested by means of an abductive derivation for them
 
        does not write input files for ICL

	modified the bias so that it does not add a literal to a clause
	if it does not have any	variable in common with the current body

Note for internal use only: correspnding to old version 21bias.
*/
...
/* added for allowing backtracking on clause generation: the second best
clause is chosen for refinement 
specialize([ _BestRule |Rest],AllowedLiterals,Eplus,
	Eminus,Delta,N):-
	format("Backtracking on clause generation~N",[]),
	specialize(Rest,Clout,AllowedLiterals,Eplus,Eminus,Delta,N).
...
evaluate_new_agenda([[Rule,_V,_C,_Np,_Npa,_Nm,_Nma,_Kp,_Km,Var]|Agenda],
                AgendaNewin,
                AgendaNewout,
                Eplus,Eminus,Delta):-
        evaluate1(Value,Rule,Eplus,Eminus,Np,Nm),
        beamsize(Beamsize),
        (Np>0->
                insert_in_order([Rule,Value,no,Np,0,Nm,0,1,1,Var],
...
evaluate1(Value,Cl,Eplus,Eminus,Nplus,Nminus):-
/* used to re-evaluate clauses in the agenda, it does not calculate a
new k but uses the one evaluated before, since it does not know the previous
number of covered examples*/
        Cl=rule(H,_B),
	findall(H,member(H,Eplus),Epc),
	length(Epc,Nplus),
	findall(H,member(H,Eminus),Emc),
	length(Emc,Nminus),
        Value is Nplus /
        (Nplus + Nminus).
...
evaluate(Value,Cl,Eplus,Epluscovered,Eminus,Eminuscovered,
	...
	Cl=rule(H,B),
        separate_abd_nabd(B,[],Nabd,Abd),
	list2and(Nabd,NabdAnd),
	assert(split_rule(H,Nabd,NabdAnd,Abd)),
	functor(H,Hpred,_),
	asserta(current_eplus_tot(Eplus)),
        asserta(current_eminus_tot(Eminus)),
	ex_covered_prolog(Eplus,[],Epc,Hpred),
	length(Epc,Nplus),
	deleteall(Eplus,Epc,Epnc),
	epluscovered(Epnc,[],Epca,Deltain,Delta1,
		0,Nplusa,Hpred),
        append(Epc,Epca,Epluscovered),

	eminuscovered(Eminus,[],Eminuscovered,Delta1,Deltaout,0,Nminus,
		0,Nminusa,Hpred),
	retract_rule(Cl),
        retract(split_rule(H,Nabd,NabdAnd,Abd)),
...
ex_covered_prolog([],Epc,Epc,_Hpred):-!.

ex_covered_prolog([H|T],Epcin,Epcout,Hp):-
        \+(functor(H,Hp,_)),!,
 % example not belonging to the current target pred
        ex_covered_prolog(T,Epcin,Epcout,Hp).
...
ex_covered_prolog([H|T],Epcin,Epcout,Hp):-
	...
	ex_covered_prolog(T,Epc1,Epcout,Hp).

/* epluscovered(Eplus,Epluscoveredin,Epluscoveredout,Deltain,Deltaout,
                0,Nplus,0,Nplusa) */
epluscovered([],Epc,Epc,Delta,Delta,Na,Na,_Hpred):-!.
        % fine della ricorsione: non ci sono piy es pos da tentare

epluscovered([H|T],Epcin,Epcout,Deltain,Deltaout,Npain,Npaout,Hp):-
        \+(functor(H,Hp,_)),!,
% example not belonging to the current target pred
...
epluscovered([H|T],Epcin,Epcout,Deltain,Deltaout,Npain,Npaout,Hp):-
        ...
        der_depth(Depth),
	split_rule(H,_NabdList,Nabd,Abd),
	% optimization: the first resolution step is done
        % with the rule
        ((Nabd,permutation(Abd,Abd1),solve_abd(Abd1,[],Delta,Depth)) ->
                % covered example
                Deltain1=[[H|Delta]|Deltain],
                Npa1 is Npain +1,
                retract(already_abduced(_)),
                retract_current_tr_set,!,
                epluscovered(T,[H|Epcin],Epcout,Deltain1,Deltaout,
                        Npa1,Npaout,Hp)
        ;
                % uncovered example
                retract(already_abduced(_)),
...
eminuscovered([H|T],Emcin,Emcout,Deltain,Deltaout,Nmin,Nmout,Nmain,Nmaout,Hp):-
        ...
        asserta(already_abduced(Deltain)),
        der_depth(Depth),
	split_rule(H,Nabd,_NabdAnd,Abd),
	% optimization: the first resolution step is done
        % with the rule
	append(Nabd,Abd,Body),
        (solve_con2([Body],[not(H)],Delta1,Depth)->
        % optmization: the rule is used immediately in the consistency phase
        % in the hypothesis  that on not(H) there is only the negation ic
...
list2and([X],X):-!.
% so that ands do not terminate with true but 

list2and([],true):-!.
% but [] is translated to true
