...
/*
:- set_sc(tree_type,model).
:- set_sc(evalfn,mse).
:- set_sc(minpos,2).       % minimum examples in leaf for splitting
:- set_sc(mingain,0.01).	% toy example needs this to be low
:- set_sc(dependent,2).	% second argument of f/2 is to predicted
:- set_sc(verbosity,10).
%:- set_sc(mingain,-1e10).
*/

:- set_sc(verbosity, 2).
:- set_sc(interactive, false).
:- set_sc(i,10).
:- set_sc(clauselength,10).
:- set_sc(nodes,10000).
