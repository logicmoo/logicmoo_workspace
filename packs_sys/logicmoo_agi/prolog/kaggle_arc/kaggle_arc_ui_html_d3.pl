...
related_formula(simply_copied,i_o,[loc2D,colorlesspoints,pen,rot2D]).
related_formula(simply_moved,i_o,[pen,differ(loc2D),rot2D,colorlesspoints]).
related_formula(simply_recolored,i_o,[differ(pen),loc2D,rot2D,colorlesspoints]).
related_formula(simply_rotated,i_o,[pen,near(loc2D),differ(rot2D),colorlesspoints]).
related_formula(simply_resized,i_o,[pen,near(loc2D),rot2D,norm_grid,differ(norm_ops)]).
...
'<script>
  ...
  top.intoNamedImg(prev,oid,100,100);
</script>',[OID]).

into_gui_item(Obj,S):- obj_to_oid(Obj,OID),global_grid(Obj,Grid),
...
%  
into_th(ColOrRow,Nth,Obj,TH):- obj_to_oid(Obj,OID),into_gui_item(Obj,S),sformat(TH,'<th id="th_~w" onclick="top.sortTable~ws(this,~w);" class="th-sm selectable">~w</th>',[OID,ColOrRow,Nth,S]).

write_i_o_data(DIR,O,I):- 
 must_det_ll((
...
write_data_between(_DIR,I,O):- I==O,  !,
  Title = I,
  write_te(Title), 
  write(' onclick="top.showDesc(this);">'),  
  write(0),
  write('</td>'),
  !.
...
write_data_between(DIR,I,O):- 
 must_det_ll((
  ...
  write_te(Title), write(' onclick="top.showDesc(this);">'),
...
sort_some_relations(DIR,IL,OL):- 
 ...
 w_section(title(Title),
  (
    ...
    format_s(`<div id="~w_panel" class="sort_some_relations" style="min-height: 200"></div>`,[IDDIR]),
    get_relation_between(DIR,Nodes,Links),
    sformat(S,'<script>top.runD3Sim("~w_panel",~@,~@)</script>',[IDDIR,
                             json_write(current_output,Nodes),json_write(current_output,Links)]),
    write(S),
    u_dmsg(S))),
  !.
...
get_relation_between(DIR,Nodes,Links):-
  findall_set(OID,(relation_between(DIR,OID1,OID2,Val),member(OID,[OID1,OID2])),OIDSet),
  findall(Node,(member(OID,OIDSet),oid_to_node(OID,Node)),Nodes),
  findall(_{source: OID1, target: OID2, value: Val},
     relation_between(DIR,OID1,OID2,Val),Links).

oid_to_node(OID,_{id:OID,name:OID,desc:Glyph,color:HTMLColor,shape:circle,size:10}):- 
   oid_to_obj(OID,Obj),object_glyph(Obj,Glyph),color(Obj,[cc(Color,_)|_]),into_html_color(Color,HTMLColor).
...
guess_some_relations(Title,IL,OL):- wants_html,!,
 w_section(title(Title),
  (
    sformat(DIR,'~w_guess_some_relations',[Title]),

    format_s(`<p>To display 'compare_objects' click <a href = "javascript:void(0)" onclick = "top.setVisible(document.getElementById('~w_panel'),'block')">here</a></p>`,[DIR]),	
    format_s(`<div id="~w_panel" class="white_content">This is the panel content.`,[DIR]),
    format_s(`<a href = "javascript:void(0)" onclick = "top.setVisible(document.getElementById('~w_panel'),'none')">Close</a>`,[DIR]),
    format_s(`<table id="~w" class="compare_objects sorttable sortable searchable display table table-bordered table-sm"><tr><th><h3>~w</h3></th>`,[DIR,Title]),
    forall(nth1(M,IL,I),(into_th('Row',M,I,ITH),write_eh(ITH))),write('</tr>'),
    forall(nth1(N,OL,O),
       (write('<tr>'),into_th('Col',N,O,OTH),write_eh(OTH),
        forall(member(I,IL),write_i_o_data(DIR,O,I)),
        write('</tr>'))),
  format_s(`</table><a href = "javascript:void(0)" onclick = "top.setVisible(document.getElementById('~w_panel'),'none')">Close</a></div>`,[DIR]))),
  !.
