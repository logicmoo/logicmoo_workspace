(define (situation transplan-sit-1)
   (:domain transplan)
...
(define (problem transplan-prob-1)
   ...
   (:situation transplan-sit-1)
   (:objects train1 - train
	     tc1 tc2 tc3 tc4 tc5 tc6 tc7 - traincar
	     truck1 - truck
	     lv1 - package)
   (:init (current-value elapsed-time 0)
...
(defvar soln-transplan-prob-1
   (list '((attach-train-car train1 tc1)
	   (lower-ramp tc1)
	   (load-livestock lv1 tc1)
	   (raise-ramp tc1)
	   (move train1 london-station bombay-station railroad1)
	   (lower-ramp tc1)
	   (unload-livestock lv1 tc1)
	   (raise-ramp tc1))
	 '((transport lv1 london-station bombay-station))))
