...
method(
 ...
 % 6. decomposition
      [ achieve( ss(package, P,[waiting(P),certified(P)]) ), carry_direct2(P,O,D), deliver(P,D)]
 ).
...
method(
      ...
      [ achieve( ss(package, P,[waiting(P),certified(P)]) ), 
        carry_direct2(P,O,A1), carry_via_ap2(A1,A2,P), 
        carry_direct2(P,A2,D), deliver(P,D)]
 ).
...
operator( move2(V, O, L, local_roads), 
         [],
         [sc(truck,V,[at(V,O),
             moveable(V),
             in_city(O,City),
