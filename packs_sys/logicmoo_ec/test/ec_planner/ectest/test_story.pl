...
*/
do_ec_test(test_story_1) :-  local_demo([holds_at(directlyIn(lisa,livingRoom),t)]).
do_ec_test(test_story_2) :-  local_demo([holds_at(inRoom(lisa,livingRoom),t)]).
do_ec_test(test_story_3) :-  local_demo([holds_at(directlyIn(lisa,kitchen),t)]).
do_ec_test(test_story_4) :-  local_demo([holds_at(inRoom(lisa,kitchen),t)]).
do_ec_test(test_story_5) :-  local_demo([holds_at(directlyIn(box,kitchen),t)]).
