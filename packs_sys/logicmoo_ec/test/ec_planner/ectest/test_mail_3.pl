...
axiom(happens(retrieve_pack(Agnt,P),T1,T2),
     ...
     not(clipped(T1,atRoom(Agnt,R),T2))]).
...
axiom(initiates(pick_up(Agnt,P),got(Agnt,P),T),
     [diff(P,Agnt), holds_at(inRoom(P,R),T), holds_at(atRoom(Agnt,R),T)]).
...
axiom(initiates(put_down(Agnt,P),inRoom(P,R),T),
     [diff(P,Agnt), holds_at(got(Agnt,P),T), holds_at(atRoom(Agnt,R),T)]).

axiom(terminates(put_down(Agnt,P),got(Agnt,P),T),[]).


axiom(initiates(go_to_room(Agnt,R),atRoom(Agnt,R),T),[]).

axiom(terminates(go_to_room(Agnt,R1),atRoom(Agnt,R2),T),[diff(R1,R2)]).
...
axiom(initially(atRoom(Agnt,r1)),[]).

axiom(initially(neg(atRoom(Agnt,r2))),[]).

axiom(initially(neg(atRoom(Agnt,r3))),[]).
...
% EDIT: https://github.com/logicmoo/logicmoo_workspace/edit/master/packs_sys/logicmoo_ec/test/ec_planner/ectest/test_mail_3.pl 
% JENKINS: https://jenkins.logicmoo.org/job/logicmoo_workspace/lastBuild/testReport/logicmoo.ec.ec_planner.ectest/TEST_MAIL_3/ 
% ISSUE_SEARCH: https://github.com/logicmoo/logicmoo_workspace/issues?q=is%3Aissue+label%3ATEST_MAIL_3 
