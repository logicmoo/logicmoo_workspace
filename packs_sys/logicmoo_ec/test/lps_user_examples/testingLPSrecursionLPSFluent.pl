...
fluents	dont_touch/1, contaminated/4, met/3. % isolate Person

events verify, meets/2. 
...
contaminated(A, Path, Path, (T1, T2)) if
   observe tested(A, positive) from T to _,  
   five_days_before(T1, T), 
   two_week_after(T, T2). 
...
%              observe([meets(B, A)], T). 
meets(A,B) from T to _ initiates met(A,B,T). 
meets(A,B) from T to _ initiates met(B,A,T). 
