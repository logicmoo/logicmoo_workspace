...
/*
   ...
   that follows from HA, BA is a list of b atoms, and BC is the transitive
   closure of BA.
*/
first_d(1000).
/*
...
/*
   ...
   updating the transitive closures of the b and beq facts in the residue.
...
/*
   ...
   This almost restores the state of computation to the state S b the chosen
   action was added to the residue in the first place. But not quite, because
   some derived b facts will be retained in the transitive closure part
   of the b part of the residue that weren't there in state S. This doesn't
   matter, however, because these derived facts will need to be there eventually
   anyway, when those b facts that have been transferred from residue to goal
