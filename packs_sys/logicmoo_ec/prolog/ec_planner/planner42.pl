...
abresolve(b(X,Y),R,[],R) :- demo_before(X,Y,R).

abresolve(b(X,Y),R1,[],R2) :-
...
demo_before(X,Y,R,L) :- X \= 0, member(b(X,Y),R).

demo_before(X,Y,R,L) :- X \= 0, \+ member(b(X,Y),R), member(X,L).

demo_before(X,Y,R,L) :-
     X \= 0, \+ member(b(X,Y),R), \+ member(X,L),
     member(b(X,Z),R), demo_before(Z,Y,R,[X|L]).
...
add_before(X,Y,[HA,BA]) :- member(b(X,Y),BA).

add_before(X,Y,[HA,BA],[HA,[b(X,Y)|BA]]) :-
     \+ member(b(X,Y),BA), \+ demo_beq(Y,X,[HA,BA]).
