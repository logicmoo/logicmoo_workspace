...
% 060527 Marco Gavanelli, use uf unnamed variables to remove "singleton variables" warning
...
gt(A,B) :- lt(B,A).				% constraints gt/2,ge/2
ge(A,B) :- le(B,A).
...
lt(A,B) <=> domain(number),ground(A),\+ number(A) | A1 is A, lt(A1,B).
lt(B,A) <=> domain(number),ground(A),\+ number(A) | A1 is A, lt(B,A1).
le(A,B) <=> domain(number),ground(A),\+ number(A) | A1 is A, le(A1,B).
le(B,A) <=> domain(number),ground(A),\+ number(A) | A1 is A, le(B,A1).
ne(A,B) <=> domain(number),ground(A),\+ number(A) | A1 is A, ne(A1,B).
ne(B,A) <=> domain(number),ground(A),\+ number(A) | A1 is A, ne(B,A1).
% use built-ins to solve the predicates if arguments are known
lt(A,B) <=> ground(A),ground(B) | (domain(number) -> A < B ; A @< B).
le(A,B) <=> ground(A),ground(B) | (domain(number) -> A =< B ; A @=< B).
ne(A,B) <=> ground(A),ground(B) | (domain(number) -> A =\= B ; A \== B).
...
/* I wrote this! And it works!! MarcoA */
