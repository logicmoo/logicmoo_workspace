...
maxRealTime(180). % 3 minutes 

observe init from 1 to 2.
/* Players will play... via web instead. 
After serve(ID) (and assuming ID is lps1, otherwise replace it below), use any browser on the web
...
http://lcdemo.logicalcontracts.com/lps_server/events/lps1?events=[player_input(bob,rock,1000)]
instead of
