...
make_options(Options, Opts) :-
	...
	make_opts(Indent, Space, _Convert, Style, Opts).
...
make_space_preserve_opts(Opts, Opts1):-
	...
	get_opt_convert_to_utf_8(Opts, E),
...
%
% This shoud mean, that "around these tags whitespace can be inserted".
...
xml_quote(In, _Opts, Out) :-
	xml_quote_cdata(In, Out).

write_xml_quote(In, _Opts) :-
	xml_quote_cdata(In, Out),
	write(Out).
