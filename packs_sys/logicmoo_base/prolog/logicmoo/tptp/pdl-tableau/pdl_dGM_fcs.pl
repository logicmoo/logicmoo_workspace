...
%  Modified 2009-10, Ullrich Hustadt, University of Manchester
%
% Uses
%        X_{<a*>p}
%   (X)--------------
...
%       This version of the calculus uses full complement splitting. So,
%       not(A) is added to the right branch.
    get_branch_point_counter(Count),
    pdl_on_write('In reduce_local, RIGHT branch of OR, reduce_local: '), pdl_on_write(Count), pdl_on_nl,
    increment_branch_point_counter(-1),
    complement(A, NotA),
    extend_unexpanded([NotA,B], Workedoff, Unexpanded, NewUnexpanded, 
                 X, ['or-RIGHT',or(A,B)]), 
    reduce_local(NewUnexpanded, X, [or(A,B)|Workedoff], 
            NewWorkedoff, Universal, Existential, Eventualities).
...
% this point and so does this version of the calculus.
    complement(A, NotA),
    get_branch_point_counter(Count),
