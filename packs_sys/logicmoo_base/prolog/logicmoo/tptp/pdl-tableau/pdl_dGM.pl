...
eventualities([pr(State,FmlList)|Branch], Eventualities) :-
    eventualities_fml(State,FmlList, Eventualities1), !,
...
eventualities([isCopyOf(_,_)|Branch], Eventualities) :-
    eventualities(Branch, Eventualities).

eventualities_fml(_,[], []).

eventualities_fml(State, [Fml|FmlList], Eventualities) :-
    eventualities_fml(State, Fml, Eventualities1), !,
    eventualities_fml(State, FmlList, Eventualities2),
    sort(Eventualities1, SortedEventualities1),
    sort(Eventualities2, SortedEventualities2),
    merge_set(SortedEventualities1, SortedEventualities2, Eventualities).
    
eventualities_fml(State, x(X,Fml), [ev(State,x(X,Fml))|Eventualities]) :-
    eventualities_fml(State, Fml, Eventualities).
...
%       rule below. 
    ...
    reduce_local(NewUnexpanded, X, [or(A,B)|Workedoff], 
            NewWorkedoff, Universal, Existential, Eventualities, [l|IDT], ODT).
...
%       of the search space / tableau tree on which B is true.
    ...
    extend_unexpanded([B], Workedoff, Unexpanded, NewUnexpanded, 
                 X, ['or-RIGHT',or(A,B)]), 
    reduce_local(NewUnexpanded, X, [or(A,B)|Workedoff], 
            NewWorkedoff, Universal, Existential, Eventualities, [r|IDT], ODT).
...
reduce_local([or(A,B)|Unexpanded], _X, Workedoff, _NewWorkedoff,
             _Universal, _Existential, _Eventualities, _IDT, _ODT) :- 
    !,
...
reduce_local([dia(or(R,S),A)|Unexpanded], X, Workedoff, NewWorkedoff,
        Universal, Existential, Eventualities, IDT, ODT) :- % no cut
    get_branch_point_counter(Count),
...
reduce_local([x(Y,dia(star(R),A))|Unexpanded], X, Workedoff, NewWorkedoff,
        ...
        [fulfilled(X, x(Y,dia(star(R),A)))|Eventualities], IDT, ODT) :- % no cut
    get_branch_point_counter(Count),
...
% this point.
    ...
    increment_branch_point_counter(-1),
    extend_unexpanded([NotA,dia(R,x(Y,dia(star(R),A)))], Workedoff, 
...
reduce_local([dia(R,A)|Unexpanded], X, Workedoff, NewWorkedoff,
        Universal, [dia(R,A)|Existential], Eventualities, IDT, ODT) :- !,
    atom(R),  % important
...
reduce_local([not(dia(R,A))|Unexpanded], X, Workedoff, NewWorkedoff, 
        [not(dia(R,A))|Universal], Existential, Eventualities, IDT, ODT) :- !,
    atom(R),  % important
