 ...
 *
 */
showHierarchy:- ignore(((member(Type,['concepts', 'roles']),showHierarchy(Type),fail))).
showHierarchy(Type) :-
	getCurrentEnvironment(EnvName),
...
clearEnvironment(EnvName) :-
	...
	retractallEnv(Env,in/9),
	retractallEnv(Env,kb_in/10),
	retractallEnv(Env,eq/9),
	retractallEnv(Env,constraint/8),
	retractallEnv(Env,rel/5),
	retractallEnv(Env,closed/5),
	retractallEnv(Env,compiledPredicate/2),
	retractallEnv(Env,conceptElement/7),
	retractallEnv(Env,conceptEqualSets/6),
	retractallEnv(Env,conceptHierarchy/3),
	retractallEnv(Env,conceptName/4),
	retractallEnv(Env,conceptSubsets/6),
	retractallEnv(Env,environment/3),
	retractallEnv(Env,given_change/4),
	retractallEnv(Env,given_inflLink/4),
	retractallEnv(Env,modalAxioms/6),
	retractallEnv(Env,roleAttributes/5),
	retractallEnv(Env,roleDefault/4),
	retractallEnv(Env,roleDefNr/4),
	retractallEnv(Env,roleDomain/4),
	retractallEnv(Env,roleElement/8),
	retractallEnv(Env,roleEqualSets/6),
	retractallEnv(Env,roleHierarchy/3),
	retractallEnv(Env,roleName/4),
	retractallEnv(Env,roleNr/5),
	retractallEnv(Env,roleRange/4),
	retractallEnv(Env,roleSubsets/6),
	retractallEnv(Env,sub/4),
	retractallEnv(Env,succ/4),
	retractallEnv(Env,abductiveDerivation/3),
...
%- fss.env
%  This is what you get if you save the environment fssEnv.
...
prolog_statistics_time(A) :-
  prolog_statistics:((
        time_state(B),
        (   call_cleanup(catch((user:A,dmsg(prolog_statistics_time(A))),
                               C,
                               (report(B, 10), throw(C))),
                         D=true),
            time_true(B),
...
% prolog_statistics_time(G):-prolog_statistics:time(user:G).

setupTest(N,Body):- dmsg(runOneExample(N)),!,must(once(Body)),dmsg(succeed(Body)).
runTest(N,Body):- dmsg(runTest(N)),!,must(once(Body)),dmsg(succeed(Body)).
...
runOneExample(N):-  clause(testMotelExample(N) , no_goal),!.
runOneExample(N):-  nl,  nl, 
   initializeMotel,
   dmsg(runOneExample(N)), nl, 
   listing(example(N)),
   listing(testMotelExample(N)),
   prolog_statistics_time(must(once(example(N)))),
   showHierarchy,
   prolog_statistics_time(forall(clause(testMotelExample(N),Body),ignore(runTest(N,Body)))).
...
% examples.pl  mpred_code_unused.txt  mpred_expansion.pl.txt  mpred_mote_examples_76l.pl
  mpred_motel_env.pl  mpred_motel_examples_63.pl  mpred_motel_fssKb.pl 
   mpred_motel.pl  mpred_pfc_d.unused  mpred_pfc_utils.unused  testMotel.pl



   ensure_loaded(library('logicmoo/motel/mpred_motel')).

   testMotel.



 :-  ensure_loaded(library('logicmoo/motel/mpred_motel_fssKb')).
 % :-  ensure_loaded(library('logicmoo/motel/mpred_motel_env')).
 :-  ensure_loaded(library('logicmoo/motel/examples')).
 :-  ensure_loaded(library('logicmoo/motel/testMotel')).
