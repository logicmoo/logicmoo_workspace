...
%
to_poss(_KB,X,poss(X)):- is_ftVar(X).
to_poss(_KB,poss(X),poss(X)).
to_poss(_KB,X,poss(X)).

to_nesc(_KB,X,nesc(X)):- is_ftVar(X).
to_nesc(_KB,nesc(X),nesc(X)).
to_nesc(_KB,X,nesc(X)).
...
nnf1(KB,nesc(F),FreeV,BOX,Paths):- 
   nnf(KB,F,FreeV,NNF,Paths), cnf(KB,NNF,CNF), boxRule(KB,nesc(CNF), BOX),!.
...
nnf1(KB,poss(PQ),FreeV,DIA,Paths):- fail, compound(PQ),PQ = (P=>Q), !,
   nnf1(KB,poss(P)=>Q,FreeV,DIA,Paths).

nnf1(KB,poss(F),FreeV,DIA,Paths):- 
   nnf(KB,F,FreeV,NNF,Paths), dnf(KB,NNF,DNF), diaRule(KB,poss(DNF), DIA).
...
nnf1(KB, ~( Fml),FreeV,NNF,Paths):- nonvar(Fml),   
      (Fml = (beliefs(~(F))) -> Fml1 = knows( F);
       Fml = (knows(~(F))) -> Fml1 = beliefs( F)
	),!,
...
nnf1(KB,release(CurrentPsi,ReleaserPhi),FreeV,NNF,Paths):- 
...
nnf1(KB,~(until(Future,Current)),FreeV,NNF,Paths):- 

   nnf(KB, ~( Future),FreeV,NNFuture,_),
   nnf(KB, ~( Current),FreeV,NNCurrent,_),
   Fml1 = v(always(NNCurrent), until(NNCurrent,&(NNFuture,NNCurrent))),
   nnf(KB,Fml1,FreeV,NNF,Paths).
   
% ~(cir)(Future) -> cir(~(Future))
nnf1(KB,~(cir(Future)),FreeV,NNF,Paths):- 
...
nnf1(KB,temporallySubsumes(TIMESPAN,TRUTH),FreeV,NNF,Paths):-  
 nnf(KB,(until(TRUTH,~(TIMESPAN))&until(~(TRUTH),TIMESPAN)),FreeV,NNF,Paths).
...
nnf1(KB, ~( Fml),FreeV,NNF,Paths):- nonvar(Fml),   
        ...
        (Fml = ( ~( A)) -> must(double_neg(KB,A,Fml1));
         Fml = (nesc(F)) -> Fml1 = poss( ~( F));
	 Fml = (poss(F)) -> Fml1 = nesc( ~( F));

	 Fml = (cir(F)) -> Fml1 = cir( ~( F));
	 Fml = (until(A,B)) -> 
            (nnf(KB, ~( A),FreeV,NNA,_), nnf(KB, ~( B),FreeV,NNB,_),Fml1 = v(always(NNB), until(NNB,&(NNA,NNB))));
...
%breakup_nnf(KB,eventually(F),X) :- !, breakup_nnf(KB,until(true,F),X).
%breakup_nnf(KB,~(eventually(F)),X) :- !, breakup_nnf(KB,~(until(true,F)),X).

breakup_nnf(_,Y,Y):- \+ compound(Y),!.
breakup_nnf(_,Y,Y):- is_ftVar(Y),!.
breakup_nnf(KB,cir((X & Y)),(cir(Xp) & cir(Yp))) :- ! , breakup_nnf(KB,X,Xp), breakup_nnf(KB,Y,Yp).
breakup_nnf(KB,cir(X),cir(Xp)) :- !,  breakup_nnf(KB,X,Xp).

breakup_nnf(KB,until((X & Y),Z),(U & V)) :- !, breakup_nnf(KB,until(X,Z),U), breakup_nnf(KB,until(Y,Z),V).
breakup_nnf(KB,until(X,(Y v Z)),(U v V)) :- !, breakup_nnf(KB,until(X,Y),U), breakup_nnf(KB,until(X,Z),V).
breakup_nnf(KB,until(X,Y),until(Xp,Yp)) :- !, breakup_nnf(KB,X,Xp), breakup_nnf(KB,Y,Yp).                                        

breakup_nnf(KB,release((X v Y),Z),(U v V)) :- !, breakup_nnf(KB,release(X,Z),U), breakup_nnf(KB,release(Y,Z),V).
breakup_nnf(KB,release(X,(Y & Z)),(U & V)) :- !, breakup_nnf(KB,release(X,Y),U), breakup_nnf(KB,release(X,Z),V).
breakup_nnf(KB,release(X,Y),release(Xp,Yp)) :- !, breakup_nnf(KB,X,Xp), breakup_nnf(KB,Y,Yp).
...
mnf(poss(A=>B),Out):- diaRule(_,poss(v( ~(-,B),A)),M),mnf(M,Out).
mnf(nesc(A=>B),Out):- mnf(v( ~(-,nesc( B)), nesc(A)),M),mnf(M,Out).
...
boxRule(KB,nesc((A & B)), (BA & BB)):- nonvar(A),!, boxRule(KB,nesc(A),BA), boxRule(KB,nesc(B),BB).
% boxRule(KB,nesc( IN), BOX):- \+ is_lit_atom(IN), share_scopes(KB,BDT), nnf(KB, ~( nesc(  ~( IN))),BOX).
boxRule(_KB,BOX, BOX).
...
diaRule(KB,poss((A v B)), v(DA,DB)):- !, diaRule(KB,poss(A),DA), diaRule(KB,poss(B),DB).
% dmiles thinks this is ok
% diaRule(KB,poss((A & B)), &(DA,DB)):- !, diaRule(KB,poss(A),DA), diaRule(KB,poss(B),DB).
diaRule(_KB,DIA, DIA).
...
cirRule(KB,cir((A v B)), v(DA,DB)):- !, cirRule(KB,cir(A),DA), cirRule(KB,cir(B),DB).
cirRule(KB,cir((A & B)), &(DA,DB)):- !, cirRule(KB,cir(A),DA), cirRule(KB,cir(B),DB).
cirRule(_KB,CIR, CIR).
...
corrected_modal0(_,nesc(F),nesc(F)):-!.
corrected_modal0(_,poss(F),poss(F)):-!.
corrected_modal0(_,until(A,B),until(A,B)):-!.
corrected_modal0(_,cir(F),cir(F)):-!.
...
pfn4(KB,   poss(F),Vs,   poss(PNF)):- !, pfn4(KB,F,Vs, PNF),!.
...
% invert_modal(KB,A,poss(A)):- is_using_feature(default_nesc),set_is_lit(A),!.
% invert_modal(KB,A,A):-!.
...
/*
removeQ(KB,  ~( nesc(  ~( F))),Vars, XF):- !,removeQ_LC(KB, poss( F),Vars, XF).
removeQ(KB,  ~( poss(  ~( F))),Vars, XF):- !,removeQ_LC(KB, nesc( F),Vars, XF).

removeQ(KB,  ~( nesc( (F))),Vars, XF):- !,removeQ(KB, poss(  ~( F)),Vars, XF).
removeQ(KB,  ~( poss( (F))),Vars, XF):- !,removeQ(KB, nesc(  ~( F)),Vars, XF).
*/
