...
:- set_sc(typeRegularization,3). % set the type of regularization
:- set_sc(gamma,0). % set the value of gamma according to the type  (gamma=a if type=3)
:- set_sc(gammaCount,0.2). % set the value of gamma according to the type (if o gamma is unchanged during the regularization) (gammaCount=b if type=3)
%:- set_sc(a,0).
%:- set_sc(frac_b,0.1).
