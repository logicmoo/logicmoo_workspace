...
export GLOB="$*"
[ -z "$GLOB" ] && GLOB="*_01.*"
GLOBSTEM=$(echo "${GLOB}-${JUNIT_PACKAGE}" | sed -e "s/[*]/vSTARv/g" -e "s/[?]/vQUESTv/g" -e "s/[^_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz]/-/g" -e "s/--/-/g" -e "s/-/-/g"  -e "s/--/-/g"  | rev | cut -c 1-110 | rev)
