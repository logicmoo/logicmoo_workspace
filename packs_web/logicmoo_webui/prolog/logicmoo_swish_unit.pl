...
:- module(logicmoo_swish_unit,
          [ unit_test_file/1
          ]).

unit_test_file(O):-source_file_property(Source, modified(_)),unit_from_source(Source,O).

unit_from_source(Source,O):- atom_concat(Source,t,O),exists_file(O).
unit_from_source(Source,O):- member(T,[test,t]),atom_subst(Source,prolog,T,O),O\=Source,exists_file(O).
unit_from_source(Source,O):- member(T,[test,t]),atom_subst(Source,prolog,T,M),atom_concat(M,t,O),O\=Source,exists_file(O).
% unit_test_file(O):-source_file_property(Source, modified(_)),atom_concat(Source,t,O),exists_file(O).
