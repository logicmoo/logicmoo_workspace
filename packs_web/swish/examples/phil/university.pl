...
/** <examples>
?- induce_hplp([f1,f2,f3,f4],P),test_hplp(P,[f5],LL,AUCROC,ROC,AUCPR,PR). % learn the structure and the parameters and test the result
...
:- set_hplp(megaex_bottom,1). % max number of mega examples to considered in the generation of bottoms clauses
:- set_hplp(initial_clauses_per_megaex,1).
:- set_hplp(use_all_mega_examples,no).
:- set_hplp(rate,0.8). % defines the probabilityu for going from the first layer to the second layer
:- set_hplp(max_layer,4). % defines the max number of layer: -1 for the maximum depth possible 
...
:- set_hplp(adam_params,[0.001,0.9,0.999,1e-8]).
...
%rating(A,B) :-rating(A,B,rating_1).
%neg(rating(A,B)) :- rating(A,B,rating_2).
begin(model(f1)).
