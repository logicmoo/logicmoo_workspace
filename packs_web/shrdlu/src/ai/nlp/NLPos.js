var PartOfSpeech = /** @class */ (function () {
    function PartOfSpeech(t, sn, term, p) {
        this.token = null;
        this.negatedSort = false;
        this.sortName = null;
        this.term = null;
    ...
    PartOfSpeech.prototype.equals = function (pos) {
        if (this.token != pos.token)
            return false;
        if (this.negatedSort != pos.negatedSort)
            return false;
        if (this.term == null) {
            if (pos.term != null)
                return false;
            ...
            if (pos.term == null)
                return false;
            if (this.term.equalsNoBindings(pos.term) != 1)
                return false;
        }
        return true;
    };
    PartOfSpeech.prototype.toString = function () {
        return this.term.toString();
    };
    PartOfSpeech.fromPartOfSpeech = function (pos) {
        return new PartOfSpeech(pos.token, (pos.negatedSort ? "~" : "") + pos.sortName, pos.term, pos.probability);
    };
    return PartOfSpeech;
}());
var TokenizationElement = /** @class */ (function () {
    function TokenizationElement(token) {
        this.POS = null;
        this.next = [];
        this.token = token;
    }
    // throught he first path
    TokenizationElement.prototype.tokensLeftToParse = function () {
        if (this.token == null)
            return 0;
    ...
    TokenizationElement.prototype.toStringSimple = function () {
        var str = this.token;
        if (this.next != null && this.next.length > 0) {
    ...
    TokenizationElement.prototype.toString = function () {
        return this.toStringInternal(0);
    };
    TokenizationElement.prototype.toStringInternal = function (tabs) {
        var s = "";
        if (this.token == null) {
            ...
            if (this.POS != null) {
                s += " [ ";
                for (var _b = 0, _c = this.POS; _b < _c.length; _b++) {
                    var pos = _c[_b];
                    s += pos.toString() + " ";
                }
                s += "]";
            ...
            s += "\n";
            for (var _d = 0, _e = this.next; _d < _e.length; _d++) {
                var n = _e[_d];
    ...
    return TokenizationElement;
}());
var POSParser = /** @class */ (function () {
    function POSParser(o) {
        this.multitokens_plainlist = [];
        this.multitokens = {};
        this.POS = {};
        this.POSbySort = {};
        this.typeSortToEnglish = {}; // it maps to two strings: for sigular, plural
        this.propertySortToEnglish = {};
        this.relationSortToEnglish = {};
        this.verbSortToEnglish = {};
        this.nounSortToEnglish = {};
        this.pronounSortToEnglish = {};
        this.determinerSortToEnglish = {};
        this.adverbSortToEnglish = {};
        ...
        this.addTokenPOS(new PartOfSpeech("or", "#or", Term.fromString("#or()", o), 1.0));
        ...
        // these have the same symbols as the possessive determiners, so, I can convert easily:
        this.addTokenPOS(new PartOfSpeech("mine", "possessive-pronoun.i", Term.fromString("possessive-pronoun('determiner.my'[symbol],[singular],[gender],[first-person])", o), 1.0));
        this.addTokenPOS(new PartOfSpeech("ours", "possessive-pronoun.i", Term.fromString("possessive-pronoun('determiner.our'[symbol],[plural],[gender],[first-person])", o), 1.0));
        this.addTokenPOS(new PartOfSpeech("yours", "possessive-pronoun.you", Term.fromString("possessive-pronoun('determiner.your'[symbol],[singular],[gender],[second-person])", o), 1.0));
        this.addTokenPOS(new PartOfSpeech("yours", "possessive-pronoun.you", Term.fromString("possessive-pronoun('determiner.your'[symbol],[plural],[gender],[second-person])", o), 1.0));
        this.addTokenPOS(new PartOfSpeech("his", "possessive-pronoun.it", Term.fromString("possessive-pronoun('determiner.his'[symbol],[singular],[gender-masculine],[third-person])", o), 1.0));
        this.addTokenPOS(new PartOfSpeech("hers", "possessive-pronoun.it", Term.fromString("possessive-pronoun('determiner.her'[symbol],[singular],[gender-femenine],[third-person])", o), 1.0));
        this.addTokenPOS(new PartOfSpeech("theirs", "possessive-pronoun.it", Term.fromString("possessive-pronoun('determiner.their'[symbol],[plural],[gender],[third-person])", o), 1.0));
      ...
      */
        ...
        */
        ...
        for (var _i = 0, proper_nouns_1 = proper_nouns; _i < proper_nouns_1.length; _i++) {
            var proper_noun = proper_nouns_1[_i];
    ...
    POSParser.prototype.isCountable = function (word) {
        var pos_l = this.POS[word];
        if (pos_l == null)
            return true;
        for (var _i = 0, pos_l_1 = pos_l; _i < pos_l_1.length; _i++) {
            var pos = pos_l_1[_i];
            if (pos.term.functor.name == "noun" &&
                pos.term.attributes.length >= 2) {
                if (pos.term.attributes[1].sort.name == "uncountable")
                    return false;
            }
        }
        return true;
    };
    POSParser.prototype.addTokenPOS = function (pos) {
        // o.getSort(pos.sortName);  // this is just to check that we have all the sorts!
        var pos_l = this.POS[pos.token];
        if (pos_l == null)
            pos_l = [];
        pos_l.push(pos);
        this.POS[pos.token] = pos_l;
        var pos_ls = this.POSbySort[pos.token];
        if (pos_ls == null)
            pos_ls = [];
        pos_ls.push(pos);
        this.POSbySort[pos.sortName] = pos_ls;
    };
    POSParser.prototype.addStandardNounPOS = function (noun, sortName, o, multitokens) {
        if (noun.indexOf(" ") != -1) {
    ...
    POSParser.prototype.addUncountableNounPOS = function (noun, sortName, o) {
        o.getSort(sortName); // this is just to check that we have all the sorts!
        this.addTokenPOS(new PartOfSpeech(noun, sortName, Term.fromString("noun('" + sortName + "'[" + sortName + "], [uncountable])", o), 1.0));
    };
    POSParser.prototype.addStandardVerbPOS = function (sortStr, inf, present3, past, participle, gerund, modal, multitokens, o) {
        /*
        ...
        */
        ...
        if (gerund != null)
            this.addTokenPOS(new PartOfSpeech(gerund, sortStr, Term.fromString("verb('" + sortStr + "'[symbol], [grammatical-number], [no-person], [gerund-tense])", o), 1.0));
        if (participle != null) {
        ...
        if (inf != null && inf.indexOf(" ") >= 0)
            multitokens.push(inf);
        if (present3 != null && present3.indexOf(" ") >= 0)
            multitokens.push(present3);
        if (past != null && past.indexOf(" ") >= 0)
            multitokens.push(past);
        if (participle != null && participle.indexOf(" ") >= 0)
            multitokens.push(participle);
        if (gerund != null && gerund.indexOf(" ") >= 0)
            multitokens.push(gerund);
    };
    POSParser.prototype.addPhrasalVerbPOS = function (sortStr, preposition, inf, present3, past, participle, gerund, modal, multitokens, o) {
        /*
        // this messes up the parser when I set up to prefer multitokens, since "to X" prevents parsing "to" as a preposition...
        if (!modal) {
          this.addTokenPOS(new PartOfSpeech("to " + inf, sortStr, Term.fromString("verb('"+sortStr+"'[symbol], [grammatical-number], [no-person], [infinitive-tense])", o), 1.0));
        ...
        */
        ...
        if (gerund != null)
            this.addTokenPOS(new PartOfSpeech(gerund, sortStr, Term.fromString("verb('" + sortStr + "'[symbol], [grammatical-number], [no-person], [gerund-tense], '" + preposition + "'[symbol])", o), 1.0));
        if (participle != null) {
    ...
    POSParser.prototype.generateCardinalNumber = function (token, o) {
        return new PartOfSpeech(token, token, Term.fromString("cardinal('" + token + "'[number])", o), 1.0);
    };
    POSParser.prototype.tokenize = function (text) {
        var tokens = this.basicTokenization(text);
        // detect numbers: 
    ...
    POSParser.prototype.basicTokenization = function (text) {
        var normal = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRTSUVWXYZ0123456789";
        var tokenSeparators = ".,!-;:'\"?()";
        var ignorableSeparators = " \t\n\r";
        var tokens = [];
        var token = "";
    ...
    POSParser.prototype.generateMultitokenTable = function (multitokens_raw) {
        this.multitokens_plainlist = multitokens_raw;
        for (var _i = 0, multitokens_raw_1 = multitokens_raw; _i < multitokens_raw_1.length; _i++) {
            var mt = multitokens_raw_1[_i];
            this.addMultiToken(mt);
        }
    };
    POSParser.prototype.addMultiToken = function (mt) {
        var tokens_tmp = mt.split(" ");
        var tokens = [];
        for (var _i = 0, tokens_tmp_1 = tokens_tmp; _i < tokens_tmp_1.length; _i++) {
            var token = tokens_tmp_1[_i];
        ...
        if (tokens.length == 1)
            return; // not a multitoken!
        var key = tokens[0];
        var value = tokens.slice(1);
        var values = this.multitokens[key];
        if (values == null)
            values = [];
    ...
    POSParser.prototype.identifyMultiTokenWords = function (tokens) {
        if (tokens == null)
            return null;
        var tokenization = new TokenizationElement(null);
        var t = tokenization;
        var preferLongerMultiTokens = true; // preference for multitoken words! remove this line if you want all parses to be returned
        //    let preferLongerMultiTokens:boolean = false; 
        for (var i = 0; i < tokens.length; i++) {
            var options = this.multitokens[tokens[i]];
            var found = false;
            var longestMultiToken = 0;
            var bestTokenization = null;
            // console.log(options);
            if (options != null) {
                for (var _i = 0, options_1 = options; _i < options_1.length; _i++) {
                    ...
                    if (found) {
                        var newToken = tokens[i];
                        for (var _a = 0, option_1 = option; _a < option_1.length; _a++) {
                            var t_1 = option_1[_a];
                            if (t_1 == ".") {
                                newToken += t_1;
                            }
                        ...
                        if (preferLongerMultiTokens) {
                            ...
                            var t2_1 = this.identifyMultiTokenWords(tokens.slice(i + option.length + 1, tokens.length));
                            t2_1.token = newToken;
                            t.next.push(t2_1);
                        }
                    }
            ...
            var t2 = new TokenizationElement(tokens[i]);
            t.next.push(t2);
            t = t2;
        }
        return tokenization;
    };
    POSParser.prototype.tagToken = function (token, o) {
        var pos_l = this.POS[token];
        if (pos_l != null) {
            // we found it!
            var pos_l2 = [];
            for (var _i = 0, pos_l_2 = pos_l; _i < pos_l_2.length; _i++) {
                var pos = pos_l_2[_i];
                var pos2 = PartOfSpeech.fromPartOfSpeech(pos);
                pos2.token = token;
                pos_l2.push(pos2);
            }
            return pos_l2;
            ...
            // numbers:
            var pos2 = this.generateCardinalNumber(token, o);
            pos2.token = token;
            return [pos2];
        }
        else {
    ...
    POSParser.prototype.POSTagging = function (tokens, o) {
    ...
    POSParser.prototype.POSTaggingInternal = function (tokens, o) {
        if (tokens.token != null)
            tokens.POS = this.tagToken(tokens.token, o);
        for (var _i = 0, _a = tokens.next; _i < _a.length; _i++) {
            var t = _a[_i];
            this.POSTaggingInternal(t, o);
        }
    };
    POSParser.prototype.POSTaggingArray = function (tokens, o) {
        var l = [];
        for (var _i = 0, tokens_2 = tokens; _i < tokens_2.length; _i++) {
            var token = tokens_2[_i];
            var pos_l = this.tagToken(token, o);
            l.push(pos_l);
        }
        return l;
    };
    POSParser.prototype.removeFailedPOSTaggingPaths = function (tokens, failedTokens) {
        if (tokens.token != null) {
            if (tokens.POS == null || tokens.POS.length == 0) {
                if (failedTokens.indexOf(tokens.token) == -1)
        ...
        for (var _b = 0, toDelete_1 = toDelete; _b < toDelete_1.length; _b++) {
            var t = toDelete_1[_b];
            var idx = tokens.next.indexOf(t);
            if (idx >= 0)
                tokens.next.splice(idx, 1);
        }
        return atLeastOneSucceeds;
    };
    POSParser.sortIsConsideredForTypes = function (s, o) {
        for (var _i = 0, _a = POSParser.sortsToConsiderForTypes; _i < _a.length; _i++) {
            var sName = _a[_i];
            if (s.is_a(o.getSort(sName)))
                return true;
        }
        return false;
    };
    POSParser.prototype.generateTypeSortToEnglishTable = function (o) {
        for (var token in this.POS) {
            ...
            for (var _i = 0, POS_l_1 = POS_l; _i < POS_l_1.length; _i++) {
                var POS = POS_l_1[_i];
                var sort = o.getSortSilent(POS.sortName);
                if (sort != null) {
                    if (POS.term.functor.name == "noun" &&
                        POSParser.sortIsConsideredForTypes(sort, o)) {
                        var tmp = this.typeSortToEnglish[POS.sortName];
                        if (tmp == null) {
                            tmp = [null, null, null];
    ...
    POSParser.prototype.generatePropertySortToEnglishTable = function (o) {
        var propertySort = o.getSort("property");
    ...
    POSParser.prototype.generateRelationSortToEnglishTable = function (o) {
        var relationSort = o.getSort("relation");
        for (var token in this.POS) {
            var POS_l = this.POS[token];
            for (var _i = 0, POS_l_3 = POS_l; _i < POS_l_3.length; _i++) {
                var POS = POS_l_3[_i];
                var sort = o.getSortSilent(POS.sortName);
                if (sort != null) {
    ...
    POSParser.prototype.generateNounSortToEnglishTable = function () {
        for (var token in this.POS) {
            // prevent synonims to overwrite the core words to be used for NLG:  
            if (token == "person" ||
                token == "persons" ||
            ...
            var POS_l = this.POS[token];
            for (var _i = 0, POS_l_4 = POS_l; _i < POS_l_4.length; _i++) {
                var POS = POS_l_4[_i];
                if (POS.term.functor.name == "noun") {
                    var tmp = this.nounSortToEnglish[POS.sortName];
                    if (tmp == null) {
                        tmp = [null, null];
    ...
    POSParser.prototype.generateVerbSortToEnglishTable = function (o) {
        for (var token in this.POS) {
            var POS_l = this.POS[token];
            for (var _i = 0, POS_l_5 = POS_l; _i < POS_l_5.length; _i++) {
                var POS = POS_l_5[_i];
                var sort = o.getSortSilent(POS.sortName);
                if (sort != null) {
                    if (POS.term.functor.name == "verb" ||
                        ...
                        var person = null;
                        var tense = null;
                        if (POS.term.attributes[1].sort.name == "singular")
                            number = 0;
                        if (POS.term.attributes[1].sort.name == "plural")
                            number = 1;
                        if (POS.term.attributes[2].sort.name == "first-person")
                            person = 0;
                        if (POS.term.attributes[2].sort.name == "second-person")
                            person = 1;
                        if (POS.term.attributes[2].sort.name == "third-person")
                            person = 2;
                        if (POS.term.attributes[3].sort.name == "infinitive-tense")
                            tense = 0;
                        if (POS.term.attributes[3].sort.name == "gerund-tense")
                            tense = 1;
                        if (POS.term.attributes[3].sort.name == "participle-tense")
                            tense = 2;
                        if (POS.term.attributes[3].sort.name == "present-tense")
                            tense = 3;
                        if (POS.term.attributes[3].sort.name == "past-tense")
                            tense = 4;
                        if (tense != null) {
                            var idxs = [];
                            if (tense < 3) {
                                ...
                                idxs = [3 + (tense - 3) * 6];
                                var idxs2 = [];
                                for (var _a = 0, idxs_1 = idxs; _a < idxs_1.length; _a++) {
                                    var idx = idxs_1[_a];
                                ...
                                idxs = [];
                                for (var _b = 0, idxs2_1 = idxs2; _b < idxs2_1.length; _b++) {
                                    var idx = idxs2_1[_b];
                                    if (person == null || person == 0)
                                        idxs.push(idx);
                                    if (person == null || person == 1)
                                        idxs.push(idx + 1);
                                    if (person == null || person == 2)
                                        idxs.push(idx + 2);
                                }
                            }
                            var tmp = this.verbSortToEnglish[POS.sortName];
                            if (tmp == null) {
                                tmp = [null, null, null,
                            ...
                            for (var _c = 0, idxs_2 = idxs; _c < idxs_2.length; _c++) {
                                var idx = idxs_2[_c];
                                ...
                                if (POS.term.functor.name == "phrasal-verb") {
                                    tmp[idx] = token + " " + POS.term.attributes[4].value;
                                }
                                else {
                                    tmp[idx] = token;
                                }
    ...
    POSParser.prototype.generatePronounSortToEnglishTable = function (o) {
        for (var token in this.POS) {
            var POS_l = this.POS[token];
            for (var _i = 0, POS_l_6 = POS_l; _i < POS_l_6.length; _i++) {
                var POS = POS_l_6[_i];
                if (POS.term.functor.is_a(o.getSort("pronoun"))) {
                    var tmp = this.pronounSortToEnglish[POS.sortName];
                    if (tmp == null) {
                    ...
                    if (POS.term.attributes[1].sort.name == "singular")
                        numbers = [0];
                    else if (POS.term.attributes[1].sort.name == "plural")
                        numbers = [1];
                    else
                        numbers = [0, 1];
                    if (POS.term.attributes[2].sort.name == "gender-masculine")
                        genders = [0];
                    else if (POS.term.attributes[2].sort.name == "gender-femenine")
                        genders = [1];
                    else if (POS.term.attributes[2].sort.name == "gender-neutral")
                        genders = [2];
                    else
                        genders = [0, 1, 2];
                    for (var _a = 0, genders_1 = genders; _a < genders_1.length; _a++) {
                        var gender = genders_1[_a];
    ...
    POSParser.prototype.generateDeterminerSortToEnglishTable = function (o) {
        for (var token in this.POS) {
            var POS_l = this.POS[token];
            for (var _i = 0, POS_l_7 = POS_l; _i < POS_l_7.length; _i++) {
                var POS = POS_l_7[_i];
                if (POS.term.functor.is_a(o.getSort("determiner"))) {
                    var tmp = this.determinerSortToEnglish[POS.sortName];
                    if (tmp == null) {
                        tmp = [null, null];
    ...
    POSParser.prototype.generateAdverbSortToEnglishTable = function (o) {
        for (var token in this.POS) {
            var POS_l = this.POS[token];
            for (var _i = 0, POS_l_8 = POS_l; _i < POS_l_8.length; _i++) {
                var POS = POS_l_8[_i];
                var sort = o.getSortSilent(POS.sortName);
                if (sort != null) {
    ...
    POSParser.prototype.isPhrasalVerb = function (s) {
        var l = this.POSbySort[s.name];
    ...
    POSParser.prototype.sortCanBe = function (s, grammaticalFunction) {
        var l = this.POSbySort[s.name];
        if (l != null) {
            for (var _i = 0, l_2 = l; _i < l_2.length; _i++) {
                var pos = l_2[_i];
                if (pos.term.functor.is_a(grammaticalFunction))
                    return true;
            }
        }
        return false;
    ...
    POSParser.prototype.getTypeString = function (s, number) {
        var tmp = this.typeSortToEnglish[s.name];
        if (tmp == null)
            return null;
        return tmp[number];
    };
    // number: 0 = singular, 1 = plural
    POSParser.prototype.getNounString = function (s, number, tryAncestors) {
        var tmp = this.nounSortToEnglish[s.name];
        if (tmp == null) {
            if (tryAncestors) {
                var sort_l = s.getAncestors();
                for (var _i = 0, sort_l_1 = sort_l; _i < sort_l_1.length; _i++) {
                    var ts = sort_l_1[_i];
                    var word = this.getNounString(ts, 0, false);
                    if (word != null)
                        return word;
                }
            }
    ...
    POSParser.prototype.getPropertyString = function (s) {
        var tmp = this.propertySortToEnglish[s.name];
        return tmp;
    };
    POSParser.prototype.getRelationString = function (s, consierRelationVerbs) {
        var tmp = this.relationSortToEnglish[s.name];
    ...
    POSParser.prototype.getVerbString = function (s, number, person, tense) {
        var tmp = this.verbSortToEnglish[s.name];
        var idx = tense;
        if (tense >= 3) {
    ...
    POSParser.prototype.getPronounString = function (s, number, gender) {
        var tmp = this.pronounSortToEnglish[s.name];
        if (tmp == null)
            return null;
        return tmp[number][gender];
    };
    POSParser.prototype.getPronounStringString = function (s, number, gender) {
        //    console.log("s: " + s + ", number: " + number + ", gender: " + gender);
        var tmp = this.pronounSortToEnglish[s];
        if (tmp == null)
            return null;
        return tmp[number][gender];
    };
    POSParser.prototype.getDeterminerString = function (s, number) {
        var tmp = this.determinerSortToEnglish[s.name];
        if (tmp == null)
            return null;
        return tmp[number];
    };
    POSParser.prototype.getAdverbString = function (s) {
        var tmp = this.adverbSortToEnglish[s.name];
        return tmp;
    };
    POSParser.prototype.getPOSString = function (s, number) {
        var candidates = this.POSbySort[s.name];
        if (candidates == null || candidates.length == 0)
            return null;
        for (var _i = 0, candidates_1 = candidates; _i < candidates_1.length; _i++) {
            var c = candidates_1[_i];
            if (c.term.functor.name == "noun" && c.term.attributes.length == 2) {
                var numberSort = c.term.attributes[1].sort;
                if (number == 0 && numberSort.name == "singular")
                    return c.token;
                if (number == 1 && numberSort.name == "plural")
                    return c.token;
                if (number == 2 && numberSort.name == "uncountable")
                    return c.token;
            }
        }
        return candidates[0].token;
    };
    POSParser.sortsToConsiderForTypes = ["time.location",
        "space.location",
        "physical-entity",
        "abstract-entity",
        "role",
